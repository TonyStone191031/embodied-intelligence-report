...
把动作离散化为标记的核心动机，是让动作接口更自然地进入 Transformer 序列建模框架。这条路线的优点在于能够直接借用成熟的自回归训练范式；问题在于，离散化是否会破坏控制精度、引入量化误差、抹平接触细节，以及如何保证标记序列对应的动作仍然具有物理可执行性。
...
Gemini Robotics 还特别值得关注的一点，是它没有把“通用多模态能力下探到机器人”停留在云端演示层，而是很快补出了 [Google DeepMind 的端侧版本说明](https://deepmind.google/blog/gemini-robotics-on-device-brings-ai-to-local-robotic-devices/) 这条本地化分支。后者公开强调本地低时延、脱离数据网络运行、以及少量示教快速适配不同平台的能力，这说明 Google DeepMind 其实在用一组连贯动作回答两个问题：第一，通用底座是否能理解更开放的机器人任务；第二，这种能力是否可以被切分并压缩到真实硬件预算下。对研究判断而言，这比单一 benchmark 更重要，因为它把“上游大模型能力”和“下游部署责任”第一次连成了同一条产品化叙事。
