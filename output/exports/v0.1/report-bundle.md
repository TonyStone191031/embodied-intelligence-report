...
为了把这种“研究型认知操作系统”从口号压回执行接口，一个最小判断单元可以写成：

\[
J_i = (\text{claim}_i,\ \text{evidence}_i,\ \text{boundary}_i,\ \text{counterexample}_i,\ \text{update trigger}_i)
\]

这里 `claim` 代表当前判断，`evidence` 代表支撑证据，`boundary` 代表适用边界，`counterexample` 代表可能推翻判断的反例类型，`update trigger` 则代表未来哪类新论文、新产品或新政策会迫使本判断被回调。把每个较强结论都尽量写成这种结构的意义，在于后续版本升级时，不必整章重写，而可以精确地更新判断节点。
...
从执行角度看，本报告对“是否纳入主线正文”的判断也可以近似写成一个过滤函数：

\[
\mathbb{I}_{\text{mainline}}(x)=
\begin{cases}
1, & x\ \text{能够映射到物理闭环、系统接口、数据闭环、部署约束或产业判断之一}\\
0, & x\ \text{主要只是热点名词、弱耦合旁证或暂不改变主线判断的噪声增量}
\end{cases}
\]

这一定义并不意味着被排除项“没有价值”，而是意味着它们更适合先沉淀在 `research/` 中，等待后续证据增强后再进入主报告。对一个长期维护项目来说，边界管理本身就是研究质量控制。
...
在证据治理的思路上，本报告会借用系统综述与证据评估领域的若干基本原则，而不是把自己写成完全随意的长文。Cochrane Handbook 当前版（Version 6.5, 2024）的目录明确把“界定研究问题与范围”“制定纳入标准”“检索与筛选研究”“考虑偏差与利益冲突”“综合证据并解释结果”等步骤组织为研究工作流的一部分，这说明严肃的研究写作并不是从“先写结论”开始，而是从“先定义问题、再处理证据”开始；相关章节结构可参见 [Cochrane Handbook 当前版](https://www.cochrane.org/authors/handbooks-and-manuals/handbook/current)。

同样，Oxford CEBM 的证据等级体系（evidence levels）虽然来自医学证据评价，但其最有启发性的部分并不是具体分级表本身，而是一个更普遍的原则：不同来源的证据强弱不同，系统综述、较高质量研究、单项研究、案例与专家意见不应被同权对待。这一原则迁移到本报告时，意味着论文、官方技术文档、代码仓库、数据集主页、企业官网、演示视频和媒体报道必须被放在不同层级上使用，而不是统一写成“有来源所以都一样可信”；相关分级说明可参见 [Oxford CEBM 证据等级说明](https://www.cebm.ox.ac.uk/resources/levels-of-evidence/oxford-centre-for-evidence-based-medicine-levels-of-evidence-march-2009)。
...
如果把证据处理进一步压成一个可复用的研究动作，则更接近如下流程：

```python
for claim in chapter_claims:
    evidence = collect_primary_sources(claim)
    level = assign_evidence_level(evidence)
    boundary = write_scope_and_failure_mode(claim, evidence)
    decision = update_or_keep(claim, evidence, boundary)
```

这段伪代码的意义不在于把研究机械化，而在于提醒后续版本维护必须先更新证据、再更新判断，而不是先被行业叙事推着改口径。对快速变化的具身智能领域而言，这种顺序纪律尤其重要。
...
因此，本报告的目录不只是“排版结果”，而更像一个研究索引函数：新信息一旦进入视野，维护者首先要判断它属于“问题定义层”“技术主干层”“系统部署层”还是“产业判断层”，再决定它应修改事实、推断还是评价。只有目录先承担这种索引职责，正文才有机会长期保持可比性，而不是在版本迭代中不断堆叠重复内容。
...
若把版本更新写成一个最小触发流程，它更接近：

```python
if new_evidence_changes_boundary(claim) or \
   new_system_changes_interface(chapter) or \
   new_company_changes_track_panel(industry_section):
    revise_current_markdown()
    update_research_assets()
    freeze_when_scope_is_stable()
```

这说明版本维护的核心不是“定期导出新 PDF”，而是判断哪些新增证据已经足以改变正文中的接口定义、能力边界或长期跟踪字段。只有在这种意义上，版本号才真正对应研究进展，而不是文件时间戳。
...
第三组矛盾，是数据驱动需求与真实世界数据稀缺之间的冲突。早期视觉直接控制工作 [《End-to-End Training of Deep Visuomotor Policies》](https://arxiv.org/abs/1504.00702) 展示了从视觉直接学习控制策略的可能性，而 [QT-Opt](https://arxiv.org/abs/1806.10293) 则进一步说明，大规模真实机器人交互数据能够显著提升闭环抓取能力；但这类数据的采集成本、设备成本和失败代价都远高于互联网文本或图像数据。具身系统不像语言模型那样能直接从海量廉价公开语料中获益，它必须面对“数据小、场景碎、标注难、失败贵”的现实。
...
以 [PaLM-E](https://arxiv.org/abs/2303.03378) 为代表的工作提出了“具身多模态语言模型”的思路，也就是把视觉、机器人状态和语言输入统一接入同一大模型体系；[RT-1](https://arxiv.org/abs/2212.06817) 与 [RT-2](https://arxiv.org/abs/2307.15818) 则进一步把机器人控制组织成更可扩展的多任务视觉-语言-动作学习框架。这些工作并不意味着机器人问题已被解决，但它们确实重新定义了机器人感知、语义理解和动作生成之间的接口。
...
从文献脉络上看，PaLM-E、RT-1 和 RT-2 的重要性，不只在于它们各自性能如何，而在于它们共同推动了一个新的组织框架：机器人不再只是“控制器 + 感知模块”的组合，而是被越来越多地组织成视觉、语言、动作共同进入的具身多模态系统。[PaLM-E](https://arxiv.org/abs/2303.03378) 明确提出了“具身多模态语言模型”这一概念；[RT-1](https://arxiv.org/abs/2212.06817) 说明多任务 Transformer 式控制模型能够在更统一的数据组织下扩展；[RT-2](https://arxiv.org/abs/2307.15818) 则进一步展示了互联网级视觉语言知识如何迁移到机器人任务理解中。它们共同支撑的不是“问题已解”，而是“问题被重新组织”这一判断。
...
因此，本部分并不试图把传统机器人学教材完整重讲一遍，而是只聚焦那些在大模型时代仍然直接决定能力上限的基础：几何与状态表示、动力学与接触、规划与控制接口，以及状态估计与环境表征。这些内容不是“旧知识背景板”，而是后续所有现代具身系统仍然必须借用、改写或显式绕开的底层接口语言。以 [《Modern Robotics》课程主页](https://modernrobotics.org/) 为代表的经典课程，至今仍把刚体运动、运动学、动力学、轨迹生成、运动规划和控制组织成同一条系统链条，这本身就说明这些问题并未被新的模型范式取消，只是被重新嵌入了新的系统中。
...
其中，`R \in SO(3)` 表示旋转，`p \in \mathbb{R}^3` 表示平移。这个写法的重要性不在于形式优雅，而在于它允许我们用同一对象描述“一个坐标系相对于另一个坐标系的刚体变换”，从而把感知、规划和控制统一到同一套几何语言中。[《Modern Robotics》课程主页](https://modernrobotics.org/) 对 `SE(3)`、刚体运动和构型空间的系统组织，也正说明位姿表示并不是预备知识，而是机器人系统接口的基础语言。
...
在大模型时代，最容易出现的一种误读，是把几何表示看成“旧时代的手工特征”。但从系统角度看，`SE(3)`、雅可比、任务空间误差和配置空间根本不是可以被轻易替代的“表示选择”，而是具身系统对物理世界进行可计算交互的最小语言。[《Planning Algorithms》](https://lavalle.pl/planning/) 对构型空间、几何规划和带微分约束的规划所做的系统组织，进一步说明几何语言并不只用于推导教科书例题，而是直接决定规划与控制问题怎样被表述。
...
具身系统真正困难的部分，往往不是自由空间中的移动，而是接触发生之后的行为。接触问题之所以棘手，在于系统一旦接触环境，就不再只是在低维轨迹空间中移动，而是在一个受摩擦、顺应性、接触不确定性和混合约束共同控制的系统中演化。Hogan 关于阻抗控制的经典工作 [《Impedance Control: An Approach to Manipulation, Part I》](https://locomotion.csail.mit.edu/elib.cgi?b=Hogan85b) 之所以长期重要，正是因为它正面回答了“系统如何在接触时表现出合适的动态关系，而不只是死板地跟踪位置命令”这一问题。
...
力/位混合控制的基本思想是：沿某些方向精确约束位置，沿另一些方向调节力或阻抗。例如，在平面擦拭任务中，法向需要维持合适接触力，而切向则需要跟踪目标运动轨迹。Khatib 提出的任务空间表述框架，以及后续任务空间控制路线，都在试图把这种需求写成更自然的控制接口：系统不只是跟踪关节量，而是在任务空间中调节与环境交互的行为，相关原始论文可参见 [《The Operational Space Formulation》](https://khatib.stanford.edu/publications/pdfs/Khatib_1987_RA.pdf)。
...
运动规划的核心问题，是在给定系统状态、目标和约束条件下，寻找一条从初始状态到目标状态的可行路径。[《Planning Algorithms》](https://lavalle.pl/planning/) 对这一领域的组织非常清楚：从几何规划到采样规划，再到带微分约束的规划，重点并不只是“找一条路”，而是如何在高维配置空间和复杂约束下构造可执行的动作序列。
...
一条几何上可行的路径并不自动等于一条物理上可执行的轨迹。系统还必须考虑速度、加速度、加加速度、关节极限、动力学约束和执行器负载。于是，规划问题会进一步从“找路径”转向“找满足时序和动力学约束的轨迹”。这也是为什么轨迹生成与运动规划在 [《Modern Robotics》课程主页](https://modernrobotics.org/) 中是相邻而不是互不相关的主题。
...
真实系统面临的并不是静态障碍和固定场景，而是动态环境、局部观测不完整、接触约束变化和实时响应要求。Khatib 在实时避障与在线约束处理方面的工作意义重大，因为它说明规划并不总是离线求解一条完整路径然后交给控制器执行；在很多情况下，约束本身会持续进入在线动作生成过程，相关原始论文可参见 [《Real-Time Obstacle Avoidance for Manipulators and Mobile Robots》](https://khatib.stanford.edu/publications/pdfs/Khatib_1986_IJRR.pdf)。
...
MPC 则把控制推进到“显式处理约束”的层面。它真正重要的地方，不在于多了一个新缩写，而在于把有限时域优化、输入/状态约束和滚动执行组织成了可部署的闭环控制框架。
关于这一路线的理论与工程化意义，可参考 [MPC 理论与实践综述](https://authors.library.caltech.edu/records/cqy11-6np77) 与 [约束 MPC 的稳定性与最优性分析](https://doi.org/10.1016/S0005-1098(99)00214-9)。
...
控制理论在现代具身系统中的持续价值，主要不在于某一类控制器“足够经典”，而在于它提供了系统稳定性、鲁棒性和实时性的分析语言。[《Underactuated Robotics》](https://underactuated.mit.edu/) 的课程组织就清楚显示出，从李雅普诺夫分析到轨迹优化，再到反馈运动规划，控制并不是单一算法，而是处理动态系统可实现性的整套框架。
...
移动机器人与具身系统进入复杂环境之后，必须持续回答“我在哪里、环境是什么样、我是否还能信任自己的地图”。Durrant-Whyte 与 Bailey 对 SLAM 的经典总结清楚说明，同步定位与建图问题之所以成为主线，是因为机器人必须同时估计自身位置与环境结构，而两者又相互依赖，相关综述可参见 [《A Tutorial on Simultaneous Localization and Mapping》](https://ieeexplore.ieee.org/document/1638022)。
...
见：[05-经典机器人学基础映射表](docs/report/versions/v0.1/tables/05-经典机器人学基础映射表.md)
...
其中，`a_t^\ast` 是专家动作。这种写法的优点在于简单直接：不需要显式奖励函数，不需要昂贵探索，也能快速建立一个“会做基本动作”的初始策略。因此，从早期视觉运动策略到今天的大量机器人示教学习系统，行为克隆一直是最自然的入口，代表性工作可参见 [《End-to-End Training of Deep Visuomotor Policies》](https://arxiv.org/abs/1504.00702)。
...
DAgger 之所以在模仿学习脉络中如此关键，是因为它清楚指出：如果只在离线专家分布上训练，那么策略在部署时引发的状态偏移会持续累积，因此必须让训练过程看到策略自己会走到的状态，并由专家对这些状态重新标注动作。
其原始论文可参见 [DAgger：数据聚合式模仿学习](https://proceedings.mlr.press/v15/ross11a.html)。
...
其中 \(\mu(\cdot\mid s)\) 表示某个更宽的候选动作分布，第二项的作用是主动降低“数据没有支撑的动作”在 \(Q\) 函数中的估值，相关代表性工作可参见 [《Conservative Q-Learning for Offline Reinforcement Learning》](https://arxiv.org/abs/2006.04779)。从机器人角度看，这个保守项的意义不是让算法显得更复杂，而是把“不要在纸面上幻想一个现实里并不可靠的动作”直接写进优化目标。
...
这类写法的重要性在于，它把“安全不是偏好，而是硬边界”明确进了问题定义。像 [《Constrained Policy Optimization》](https://arxiv.org/abs/1705.10528) 这样的工作之所以长期被反复引用，不只是因为它提供了一个算法名字，而是因为它把“每次更新都不能把系统推到明显更危险的区域”这一工程直觉，转写成了可分析、可实现的策略更新约束。对真实机器人而言，shielding、动作裁剪、工作空间屏蔽和安全接管，很多都可以视为这个 constrained MDP 视角在系统层的工程展开。
...
### 29.3 通用技能库与技能迁移
因此，技能迁移真正困难的并不是把一个名字复用到另一台机器人，而是保持其前置条件、可观测状态和失败恢复逻辑也能迁移。一个“抓取”技能在双指夹爪、五指灵巧手和吸盘末端之间，表面任务相同，内部却可能依赖完全不同的接触判据与恢复动作。这也是为什么后续很多基础模型路线会强调技能标记、动作块、可调用接口或程序式接口：它们试图把技能从纯轨迹片段提升为“带契约的中层能力对象”，从而让迁移问题变成接口重绑定，而不只是参数复制。
技能迁移的难点在于，一个看似相同的技能在不同本体、不同夹爪、不同摩擦条件和不同任务容错要求下，往往并不等价。也就是说，技能库不是静态的“动作函数仓库”，而更像一组带有适用前提、参数接口和失败模式描述的中层能力对象。后续很多基础模型路线之所以强调技能标记、动作块或程序式接口，本质上都是在寻找一种既保留可学习性、又维持技能可迁移性的中间表示。
...
Decision Transformer 的重要性，并不只在于它把强化学习（RL）重写成序列建模，而在于它表明：一旦系统开始把轨迹、回报、状态和动作放到统一序列接口中，传统策略学习与更一般的表示学习之间的边界会开始模糊，相关论文可参见 [《Decision Transformer》](https://arxiv.org/abs/2106.01345)。这为后来的 VLA、动作 token 化和多模态统一接口提供了认知上的桥梁。换言之，机器人基础模型并不是凭空出现的，它们依赖的正是此前这些跨任务表示与统一接口思路的持续积累。
...
如果只关心控制而非重建，最后一步甚至不必显式解码成像素，而可以直接预测奖励、终止标志、接触模式或可供性。也正因为如此，潜空间模型常被进一步区分为“重建导向”和“控制导向”两类：前者更在意观测保真，后者更在意潜变量是否可用于规划、值函数估计和技能切换。
...
因此，本部分不写一章通用大模型综述，而只聚焦那些会直接决定后续 VLA、规划推理和系统架构写法的要素：序列建模、跨模态对齐、语言作为任务接口、高层推理能力，以及这些能力在动作空间与实时闭环中暴露出的新问题。Transformer 之所以成为后续一切统一路线的基础，并不只因为它“模型大”，而是因为它提供了一个足够一般的序列接口，使状态、语言、图像、动作甚至回报都能够被放在同一上下文中处理，相关原始论文可参见 [《Attention Is All You Need》](https://arxiv.org/abs/1706.03762)。
...
如果把机器人控制重写成统一序列接口，其最有价值的地方并不是“长得像语言模型输入”，而是允许不同来源的信息被放到同一个时间展开的条件分布中。一个更接近具身系统的抽象写法可以记为：

```math
x_{1:T} = [\,l_{1:L};\ o_1,\ x_1,\ a_1,\ \dots,\ o_T,\ x_T\,], \qquad
\pi_\theta(a_t \mid x_{\le t})
```

其中 \(l_{1:L}\) 是语言任务描述，\(o_t\) 是视觉或多模态观测，\(x_t\) 是本体状态。这样写的意义在于，它把“任务条件”“状态历史”“动作历史”统一进一个可条件化序列接口，而不必预先强行规定哪个模块天然高于哪个模块。Transformer 最早被系统化提出于 [《Attention Is All You Need》](https://arxiv.org/abs/1706.03762)；对机器人而言，它真正可迁移的不是机器翻译经验本身，而是这种“把不同时间步和不同模态放进同一条件分布”的组织方式。
...
一旦把机器人问题写成序列建模，就自然会触及动作 token 化问题：动作到底应被视为连续控制量、离散标记、分块动作，还是更高层的技能标记？这不是一个纯实现细节，而是决定系统怎样理解动作空间的根本选择。连续动作表示保留了物理精度，但难以直接复用语言模型接口；离散标记便于统一建模，却可能粗化控制语义；动作分块则试图在短期控制平滑性与长时程规划之间做折中。
...
那么模型输出的就不再是单步动作，而是一个局部时间窗口中的动作计划。这种写法在机器人中的吸引力在于，它能减轻逐步自回归生成造成的抖动和误差累积，也更接近很多现实控制器按小时间段执行参考轨迹的方式。ACT、Diffusion Policy 与多种分块策略都体现了这一路线的工程合理性，代表性工作可参见 [ACT](https://arxiv.org/abs/2304.13705) 和 [Diffusion Policy](https://arxiv.org/abs/2303.04137)。
...
这里真正困难的不是“压缩得更短”，而是“在压缩后仍保留对下一步决策最有价值的信息”。这也是为什么检索增强、事件记忆和任务摘要在机器人里往往比盲目扩窗口更现实，相关代表性系统可参见 [SayCan 项目页](https://say-can.github.io/) 与 [Code as Policies 项目页](https://code-as-policies.github.io/)。
...
这也是为什么很多看起来“像语言模型一样生成动作”的路线，最后仍不得不引入动作块、低层控制器、动作滤波器、安全监视器甚至 MPC 后端。它们不是对基础模型路线的背离，而是在承认一个事实：动作标记只是接口形式上的离散写法，背后仍然连着连续时间、连续状态和接触动力学约束。

从学习角度看，文本标记的语义边界往往由离散词表天然给出；动作标记的边界却常常是人为设计的。我们到底是直接预测关节增量、末端位姿、力控目标、技能标签，还是某种潜在动作编码，本身就会改写问题难度。因此，动作序列与文本序列最容易被误类比的地方，不是都能“自回归”，而是忽略了动作表示、执行器、控制频率和安全闭环共同构成了真正的问题定义。

## 33. 多模态编码、连接器与 token 接口

### 33.1 多模态大模型的基本问题设定：为什么总要经过“可供语言主干消费”的接口

围绕多模态大模型，最容易出现的一个误解是：仿佛所有模态最终都被“硬翻译成文本”，然后再交给语言模型处理。更准确的说法是，主流系统通常先用模态专用编码器把原始信号变成连续表征，再用连接器把这些表征投射到语言主干可消费的上下文向量序列中。语言主干看到的往往不是自然语言词元本身，而是一串在维度、位置编码和上下文机制上与词元兼容的向量。[BLIP-2](https://arxiv.org/abs/2301.12597)、[Flamingo](https://arxiv.org/abs/2204.14198)、[PaLM-E](https://arxiv.org/abs/2303.03378)
...
h^{(m)} = E_m(x^{(m)}), \qquad
u^{(m)} = C_m(h^{(m)}), \qquad
y = \mathrm{LM}\!\left([u^{(1)};\dots;u^{(M)};l]\right)
\]

其中 \(x^{(m)}\) 是第 \(m\) 种模态的原始输入，\(E_m\) 是该模态的专用编码器，\(C_m\) 是连接器，\(l\) 是语言上下文，\(u^{(m)}\) 则是被送入语言主干的接口表示。这个公式的重要性不在于形式新颖，而在于它点出了当前多模态系统的真正工程现实：所谓“统一”，首先是统一到一个共享的上下文接口，而不是先验保证所有模态在语义上已经天然同构。

为什么这一路线会成为主流？原因并不神秘。第一，现成的大语言模型已经拥有极强的上下文建模能力与互联网知识先验，复用它们比从零训练一个全新的任意模态主干现实得多。第二，语言主干天然提供了因果解码、长上下文、指令跟随与多任务条件化等成熟能力，机器人和多模态研究可以直接借它承接任务描述、外部知识与历史记忆。第三，接口化设计使不同模态子系统能够相对独立地升级，这对工程系统尤为重要。[LLaVA](https://arxiv.org/abs/2304.08485)、[PaLM-E](https://arxiv.org/abs/2303.03378)

但必须立刻加上一句限制：这只是当前最实用的主流解法，不是已经被理论证明的最优结构。它本质上是一种“为了复用强语言主干而建立的桥接接口”。因此，用户提出的直觉其实抓住了问题核心：多模态大模型今天确实大量依赖“先编码、再桥接、再让语言主干消费”的范式；学界真正争论的，不是这件事有没有发生，而是这种接口在什么条件下足够好、什么时候会成为瓶颈，以及有没有更原生的替代路线。

### 33.2 编码器选型：视觉、语音、触觉、状态与动作信号分别适合什么编码器

一旦承认“多模态首先是接口问题”，编码器选型就不再只是追求某个单模态 leaderboard 最优，而要回到信号类型本身。视觉输入通常是高维、空间结构强、语义层次丰富的信号，因此最常见路线是 ViT 或 CLIP 系列视觉主干；视频则要进一步处理时间维度，常见做法包括逐帧编码再做时序聚合，或者直接采用带时间建模的视觉 backbone。[CLIP](https://arxiv.org/abs/2103.00020)、[Flamingo](https://arxiv.org/abs/2204.14198)

语音或音频信号则与视觉不同。它在局部时间结构上更强，通常更适合 Conformer、语音 Transformer 或更专业的语音前端，而不是直接沿用视觉 patch 化方法。对机器人来说，这一点尤其重要，因为语音输入往往不仅承担语义指令，还承担时序提示、对话轮次、甚至现场噪声条件下的鲁棒命令接收。编码器若只优化语义识别，却没有考虑实时性、口音、环境噪声和唤醒机制，就很难进入真实控制链路。

触觉、力觉、本体状态和动作信号则又是另一类对象。它们通常维度更低，但采样频率更高、时间敏感性更强、物理含义更直接。对这类信号，机器人系统常用 MLP、1D 卷积、时序 Transformer，或干脆保留为结构化数值接口，再经过轻量投影进入上层模型。[PaLM-E](https://arxiv.org/abs/2303.03378) 将连续状态直接作为多模态输入的一部分，就体现了这种思路：对低维连续状态，未必需要过重的视觉式编码器，真正重要的是保留量纲、同步关系和控制相关性。

动作信号的编码更需要谨慎。若动作被直接视为连续向量，接口较贴近真实控制，但不易复用离散语言建模范式；若动作被离散化成 token，又会引入量化误差与物理粗化；若动作被表示成 chunk、技能 token 或潜在动作码，则又是在“表达力”和“稳定性”之间做折中。[RT-2](https://arxiv.org/abs/2307.15818)、[OpenVLA](https://arxiv.org/abs/2406.09246) 代表的正是这种思路：动作并不是一个自然给定的词表，而是必须围绕控制接口、时间粒度与本体差异被重新设计。

因此，编码器选型最该避免的误区，是把“模态统一”理解成“所有模态都应使用相同编码器”。更现实的目标是：让不同编码器在各自最适合的表征空间里工作，再在连接器处完成受约束的汇合。对于具身系统，这比表面上的结构统一更重要，因为机器人真正关心的是物理闭环是否被保住，而不是架构图是否足够优雅。

如果把这一点写成工程视角下的速查表，可以得到如下对照：

| 模态 | 常见编码器/前端 | 优先保留的结构 | 选型错配后的典型问题 |
| --- | --- | --- | --- |
| 单帧视觉 | ViT、CLIP 视觉主干 | 空间布局、目标语义、局部细节 | 语义够强但几何不稳，导致抓取或对位误差 |
| 视频/多帧观察 | 时序视觉 backbone、帧级编码后再聚合 | 时间连续性、运动线索、遮挡变化 | 看得清单帧，却读不懂动态演化 |
| 语音/音频 | Conformer、语音 Transformer | 时间局部结构、噪声鲁棒性、轮次边界 | 指令识别延迟大、噪声场景下误触发 |
| 触觉/力觉 | MLP、1D 卷积、时序 Transformer | 接触相位、微小力变化、突发事件 | 接触异常被抹平，动作稳定性下降 |
| 本体状态 | 结构化数值接口、轻量投影 | 量纲、同步关系、最新状态 | 状态被过度语义化，失去控制相关性 |
| 动作/技能 | 连续向量、离散 token、chunk、潜在动作码 | 连续性、可执行性、频率与本体匹配 | token 易复用但量化粗糙，连续控制更贴近物理但难以统一建模 |

这个表最重要的提醒不是“哪种编码器最好”，而是“哪类信号绝不能被错误抽象”。在视觉问答里，局部几何被轻微压缩也许还能容忍；但在接触插装、双足平衡或高速闭环里，错配编码器带来的不是分数下降，而是整条控制链路的不稳定。

### 33.3 连接器设计：线性投影、MLP adapter、Q-Former、Perceiver Resampler 与 latent query

连接器是当前多模态系统里最容易被低估、却最能决定系统边界的部分。它表面上只是一个“把编码器输出映射到语言主干输入维度”的桥接层，但实际上它决定了三件大事：第一，保留多少模态信息；第二，以什么压缩方式保留；第三，让语言主干以什么结构读取这些信息。

最简单的连接器是线性投影或小型 MLP adapter。这条路线的优点是便宜、稳定、容易训练，也说明在不少任务中，模态桥接问题并不一定需要极复杂结构；只要底层视觉编码足够强，简单投影也能让语言主干获得可用的视觉条件。[LLaVA](https://arxiv.org/abs/2304.08485) 就在很大程度上说明了这一点：简单投影并不自动意味着能力弱，特别是在视觉问答和指令跟随这类高层任务上。

但一旦输入模态更复杂、时空冗余更高、上下文预算更紧，简单投影就很快触顶。这也是 [BLIP-2](https://arxiv.org/abs/2301.12597) 引入 Q-Former 的原因。Q-Former 的核心思想，不是把所有视觉 token 全送入 LLM，而是让一组可学习查询先对视觉编码器做交叉注意力，抽取出固定数量、对语言主干更有价值的视觉摘要。这可以写成：

\[
u_i = \mathrm{CrossAttn}(q_i, H_{\text{vision}}), \qquad i=1,\dots,K
\]

其中 \(q_i\) 是学习到的查询，\(H_{\text{vision}}\) 是视觉特征网格，\(K\) 是输出给 LLM 的固定 token 数。这个设计的真正意义，在于它把“信息压缩”显式化了：不是无差别下采样，而是用任务可学习的查询来决定哪些视觉信息值得进入语言主干。

[Flamingo](https://arxiv.org/abs/2204.14198) 提出的重采样器路线则提供了另一条代表做法。它同样使用固定数量的潜在查询来吸收高维视觉输入，但强调在任意数量图像、视频帧与文本穿插上下文中维持稳定接口。更一般的[Perceiver IO 论文](https://arxiv.org/abs/2107.14795) 则把这种“用少量潜变量查询读写高维输入输出”的思想系统化了。对机器人来说，这条线尤其值得重视，因为视觉、深度、点云、触觉和状态历史都可能造成输入爆炸；若没有一个受控的潜变量瓶颈，系统很容易被上下文预算与时延预算同时压垮。

因此，连接器并不是“越复杂越好”，而是在回答一个更具体的问题：到底该让语言主干直接看见多少原始模态细节。线性投影更像“少做假设，尽量直连”；查询式连接器和重采样连接器更像“先做选择性读取，再把预算留给主干”；潜变量查询和潜变量槽位则把“少量受控的跨模态记忆槽位”进一步制度化。它们背后的分歧，本质上不是实现风格分歧，而是对“信息在哪里该被压缩”的分歧。

若把这种分歧写成近似复杂度，问题会更清楚。设原始模态编码后得到 \(N\) 个 token，连接器输出给主干的压缩结果为 \(K\) 个 token，主干原本已有 \(T\) 个文本或历史 token，则可粗略比较：

\[
\mathrm{Cost}_{\text{proj}} \approx O(N), \qquad
\mathrm{Cost}_{\text{cross-attn}} \approx O(KN)
\]

\[
\mathrm{Cost}_{\text{LLM-direct}} \approx O((T+N)^2), \qquad
\mathrm{Cost}_{\text{LLM-compressed}} \approx O((T+K)^2)
\]

这说明一个很关键的工程事实：复杂连接器的意义，往往不在于它自己便宜，而在于它能把原本会在主干里持续放大的上下文成本，前移为一次更受控的压缩成本。只要 \(K \ll N\)，即使连接器本身更复杂，端到端推理仍可能更省预算。对机器人而言，这种“前端多做一点结构化压缩，换下游稳定时延”的取舍，常常比纯 benchmark 分数更重要。

### 33.4 token 组织方式：prefix、interleaving、patch token、latent slot、action token

即使编码器和连接器固定了，问题仍然没有结束，因为模态表示还要被组织进具体的上下文序列。当前主流组织方式大致有五类。

第一类是前缀式组织，即把视觉或状态词元放在文本前面，作为统一上下文的前缀。这是最直观、实现也最简单的方式，特别适合“先看输入，再生成文字或动作”的自回归结构。第二类是交错式组织，即图像、视频帧、文本描述、状态更新交替出现，让模型显式感知多模态时间顺序；[Flamingo](https://arxiv.org/abs/2204.14198) 就体现了这种思想。第三类是保留图像块级词元，即尽量保留更细的空间粒度，让下游主干自己决定关注哪里；它提升空间细节，但代价是上下文长度迅速膨胀。

第四类是潜变量槽位或查询槽位。它不追求保留原始图像块细节，而是先压成一小组潜变量槽位，再把这些槽位送进主干。这类方法常常更适合机器人，因为具身系统更在意“能否保留与当前动作决策最相关的少量结构性信息”，而不是把所有视觉纹理原封不动转给上层。第五类是动作词元组织，即把动作本身也当成可序列化对象。动作词元可以是离散分箱、动作块、技能标记，甚至潜在动作码；一旦采用这种做法，策略学习就能被重写为：

\[
p_\theta(a_{t:t+H} \mid o_{\le t}, x_{\le t}, l)
\]

其中 \(a_{t:t+H}\) 可以是单步动作，也可以是一个短时动作块。像 [RT-2](https://arxiv.org/abs/2307.15818) 这样的路线，本质上就是在探索“动作能否被纳入统一 token 接口”；[OpenVLA](https://arxiv.org/abs/2406.09246) 则进一步把这种接口推向开放研究与可复现实验。

需要特别强调的是，“词元化”不等于“硬生生编码成自然语言词”。今天大多数系统的真实情况是：先把多模态信号变成连续向量，再把这些向量塞进一个以词元为基本上下文单位的主干中。因此，真正需要追问的不是“有没有词元接口”，而是“这些词元是否保留了任务所需结构”。在具身系统里，若词元组织方式破坏了接触时序、空间对位、状态新鲜度或动作连续性，那么哪怕语言主干再强，也只会在上游信息残缺的前提下推理。

从具身系统实现角度看，这个过程更接近如下伪代码，而不是“把所有模态一次性丢给同一个模型”：

```text
Inputs:
  o_t^vision, o_t^audio, x_t^state, m_t^memory, l_t^language

h_v <- E_v(o_t^vision)
h_a <- E_a(o_t^audio)
h_s <- E_s(x_t^state)
r_t <- Retrieve(m_t^memory, l_t, x_t^state)

u_v <- C_v(h_v)
u_a <- C_a(h_a)
u_s <- C_s(h_s)

z_t <- BudgetAllocate([u_v, u_a, u_s, r_t, l_t], K_ctx, Δt)
g_t <- π_high(z_t)
a_{t:t+H} <- π_low(x_t^state, g_t)
```

这段伪代码里最容易被忽视的一行，其实是 `BudgetAllocate`。它提醒我们：多模态系统的关键不只是“会不会编码”，而是“在给定上下文预算与时延预算下，最终哪些信息有资格进入主干”。对机器人来说，这一步就是许多性能差异与失败模式的真实起点。

### 33.5 信息损失、模态鸿沟与上下文预算：为什么“全都编码成串行 token”既强大又粗糙

把一切模态都纳入统一 token 接口的最大优点，是终于能用一个共享上下文来承接视觉、语言、状态、动作和记忆；但它最大的代价也同样清楚：高维连续世界被压缩成有限预算的串行表示后，信息损失几乎不可避免。对图片和视频，这种损失表现为空间细节和时间分辨率下降；对触觉和本体状态，它更危险，因为被压掉的往往不是“背景噪声”，而是动作稳定性真正依赖的微小接触与相位信息。

因此，更合理的写法不是把多模态编码视为“表达问题”，而是把它视为带预算约束的信息分配问题：

\[
\max_{\{E_m, C_m\}} \ \mathrm{Perf}_{\text{task}}
\quad
\text{s.t.}\quad
\sum_m K_m \le K_{\text{ctx}}, \ \ 
T_{\text{encode}} + T_{\text{infer}} \le \Delta t
\]

其中 \(K_m\) 是第 \(m\) 种模态占用的 token 预算，\(K_{\text{ctx}}\) 是总上下文容量，\(\Delta t\) 是系统允许的时延预算。这个形式把问题说得很直接：多模态接口的成败并不只看“会不会对齐”，还看在给定上下文和时间预算内，哪些信息被保住，哪些被牺牲。

学界对这种损失并非没有意识。早期的[模态鸿沟论文 Mind the Gap](https://arxiv.org/abs/2203.02053) 已经指出，即便视觉-语言表示被拉到同一语义空间，不同模态仍可能保留显著的几何偏移；这意味着“进入同一空间”并不等于“差异消失”。到 2025-2026 年，这种担忧进一步演化为更细的工程问题：怎样缩小模态鸿沟、怎样做词元压缩、怎样在不严重破坏能力的前提下降低上下文成本。相关代表工作包括[ReAlign 论文](https://arxiv.org/abs/2602.07026)、[TAMP 论文](https://arxiv.org/abs/2504.09897)、[TokenCarve 论文](https://arxiv.org/abs/2503.10501) 和[通用词元压缩论文](https://arxiv.org/abs/2506.01097)。

更有意思的是，近年的经验证据已经开始显示：不同任务对词元组织与压缩极度敏感。[Reading, Not Thinking 论文](https://arxiv.org/abs/2603.09095) 指出，把图像中的文本内容粗暴转写并不等价于真正看见原始视觉输入，模型在数学、版式和细粒度结构任务上会显著退化。这个结论对于机器人尤其重要，因为机器人同样处于“高维连续输入被强行压到有限接口”的处境里。若连图像中的二维排版结构都会在错误编码下受损，那么触觉、速度、接触相位这类更依赖细粒度时序结构的信号，只会更脆弱而不是更安全。

所以，从具身系统角度看，“全都编码成串行 token”之所以既强大又粗糙，原因正在于此：它强大，是因为它给了我们一个真正可统一的跨模态主干；它粗糙，是因为现实世界的几何、接触和动力学并不会自动服从这个接口的压缩偏好。后文讨论世界模型、状态估计和动作接口时，这个限制必须始终被记住。

若把这种预算分配写成更直接的失败模式表述，可以得到如下四类典型后果：

1. 过度优先语义 token，压缩几何与接触细节：结果是“会描述、不会对位”。
2. 过度保留视觉 patch，牺牲历史状态与记忆：结果是“看得多，但长时任务组织变差”。
3. 过度压缩本体状态与动作上下文：结果是“高层计划看似合理，低层执行频繁抖动或失稳”。
4. 过度依赖统一主干，缺少专用动作/状态旁路：结果是“系统语义一致，但控制回路不新鲜”。

这四类失败模式的共同点在于：它们都不是传统意义上的“模型不会推理”，而是接口预算策略本身定义错了。因此，多模态 token 化的真正争论焦点，不在于“是否统一”，而在于“统一之前，哪些结构绝不能被代价过低地抹平”。

### 33.6 经验证据、可解释性线索与能力边界：哪些问题已被论证，哪些仍主要靠经验工程

如果把现有证据分层，一个较稳妥的结论是：当前学界已经较充分地论证了“多模态接口是可行的”，但远没有论证“当前这套 token 接口已经接近最优”。像 [BLIP-2](https://arxiv.org/abs/2301.12597)、[Flamingo](https://arxiv.org/abs/2204.14198)、[LLaVA](https://arxiv.org/abs/2304.08485) 这类工作，已经用大量消融和任务结果说明：只要视觉编码器、连接器和语言主干之间的职责分工设计得当，模型可以在不完全重训主干的情况下获得显著的多模态能力。换言之，“桥接范式能跑通”这件事，已不再是猜想。

第二层证据是，连接器与词元预算并非无关紧要的实现细节，而是真正决定能力边界的结构变量。查询式连接器、重采样连接器、词元压缩路线和模态鸿沟修正路线之所以持续活跃，恰恰说明社区并不满足于“能用就行”，而是在不断寻找更少信息损失、更强条件化和更低时延的接口。对用户提出的怀疑，学界的实际回应不是否认，而是通过大量结构改良来承认这个怀疑有现实基础。

第三层证据来自可解释性与表征分析，但要坦率地说，这一层仍然偏经验科学而不是完备理论。我们有越来越多关于模态鸿沟、视觉词元重要性、压缩后退化模式和几何对齐结构的探针实验，却仍缺少能够严格证明“某种接口形式对高频控制或真实接触最优”的理论框架。[模态鸿沟论文 Mind the Gap](https://arxiv.org/abs/2203.02053)、[ReAlign 论文](https://arxiv.org/abs/2602.07026)、[Reading, Not Thinking 论文](https://arxiv.org/abs/2603.09095) 更像是在不断给出现象学证据：某些接口会系统性损失某类能力，某些桥接方式会缩小但不会消灭模态偏差。

因此，本节最重要的结论不是简单站队“统一 token 接口好”或“统一 token 接口坏”，而是把边界说清楚：

1. 这一路线已经被大量实证工作证明为当前最有效、最可扩展的多模态复用范式之一。
2. 但它首先是一种工程上成功的桥接范式，而不是被严格证明的最终形态。
3. 对机器人来说，最脆弱的部分恰恰不是高层语义，而是高频状态、接触与动作连续性的压缩表示。
4. 学界确实已经在系统性质疑和改良这套范式，改良方向包括更强连接器、token 压缩、latent 接口、专用状态/动作通路，以及更原生的多模态主干。

换句话说，用户提出的那种直觉判断只能说“对了一半”。今天的多模态大模型确实广泛依赖“先编码、再桥接、再交给语言主干”的范式；但学界并没有把这件事当成理所当然，而是在不断用实验、探针和架构改造去逼问它的真实能力边界。对具身智能而言，真正值得长期跟踪的也正是这件事：未来的突破会不会来自更大的语言主干，还是来自更聪明、更少信息损失的多模态接口本身。

若进一步按“证据强度”划分，可以把当前结论再压实一层：

1. 已被较强实证支持的部分：模态专用编码器加连接器的桥接范式可以稳定工作，而且在视觉-语言与部分机器人接口中已具备很强复用价值。[BLIP-2](https://arxiv.org/abs/2301.12597)、[Flamingo](https://arxiv.org/abs/2204.14198)、[PaLM-E](https://arxiv.org/abs/2303.03378)
2. 已被大量工程实践支持、但边界仍不稳的部分：token 压缩、latent query 和动作 token/chunk 确实能在预算内提升可用性，但不同任务、不同本体、不同控制频率下的最优接口并不统一。[RT-2](https://arxiv.org/abs/2307.15818)、[OpenVLA](https://arxiv.org/abs/2406.09246)、[TAMP](https://arxiv.org/abs/2504.09897)、[TokenCarve](https://arxiv.org/abs/2503.10501)
3. 已出现清晰预警、但还远谈不上定论的部分：模态鸿沟、视觉 token 重要性和错误串行化会系统性损害某些能力，但“哪些模态绝不能被过度串行化”仍缺少统一理论。[Mind the Gap](https://arxiv.org/abs/2203.02053)、[ReAlign](https://arxiv.org/abs/2602.07026)、[Reading, Not Thinking](https://arxiv.org/abs/2603.09095)
4. 对具身系统最值得警惕的未解问题：主干上下文如何与高频状态、动作回路和安全旁路协同。这一点至今主要还是系统工程问题，而不是已经被完备解决的模型问题。
...
因此，把 VLM 变成 VLA，从系统角度看并不是“多加一个动作头”那么简单，而是要重新回答：动作是连续回归、离散标记、动作块还是技能调用；错误由谁兜底；模型更新频率和控制器更新频率如何分层；遇到超出分布的状态时系统先停、先问人还是先回退技能。
...
若把这条桥接链写得更具体一些，现实系统往往不是直接从语义 embedding 输出最终控制量，而是经历“高层动作意图 -> 中层参数化技能 -> 低层可跟踪参考”的逐层收缩：

```math
z_{1:K} \sim \pi_{\mathrm{high}}(o_{\le t}, l), \qquad
\theta_k \sim \pi_{\mathrm{param}}(o_{\le t}, z_k), \qquad
a_{t:t+H} = \pi_{\mathrm{low}}(x_t, z_k, \theta_k)
```

这里 \(z_k\) 可以是技能 token、动作块类型或局部子目标，\(\theta_k\) 则表示抓取姿态、接近方向、力控模式、速度上界等参数。把这一层写出来非常重要，因为它说明 VLA 的真实难点并不是“会不会出动作”，而是“会不会出一种能被系统其他层稳定消费的动作接口”。
...
若把这一机制形式化，一个更贴近具身系统的检索闭环通常是：

```math
q_t=\phi(o_t, x_t, g_t, m_t), \qquad
M_t=\mathrm{Retrieve}(q_t,\mathcal{B}), \qquad
\pi(a_t \mid o_t, x_t, g_t, M_t)
```

其中 \(q_t\) 是由当前观测、本体状态、任务目标与局部记忆构成的查询，\(\mathcal{B}\) 是外部记忆库，\(M_t\) 则是被取回的任务相关片段。这个写法的重要性在于，它把“记忆”从模型参数中的隐式幻想，改写成了一个显式的运行时接口：记忆何时写入、按什么键检索、如何与当前状态融合、在多大时延预算内返回，都会直接影响系统能否在现场稳定工作。对机器人而言，外部记忆真正有价值的对象往往包括三类：环境与对象状态、失败恢复案例、以及用户与场景专属流程约束。缺少这三类记忆中的任何一类，系统都会表现出“高层很懂、现场很盲”的断裂。
...
见：[07-VLM到VLA接口差异表](docs/report/versions/v0.1/tables/07-VLM到VLA接口差异表.md)
...
见 [09-机器人感知成功标准对照表](docs/report/versions/v0.1/tables/09-机器人感知成功标准对照表.md)。
...
从谱系上看，世界模型并不是今天才出现的概念。早期代表工作已经分别沿着“潜空间建模”“想象式展开”和“抽象表征预测”三条线推进：World Models 强调内部环境想象，Dreamer 把潜动态模型与策略学习更紧密地耦合，JEPA 则把重点从像素重建转向抽象表征预测。
相关代表性工作可参见 [World Models：潜空间想象模型](https://arxiv.org/abs/1803.10122)、[Dreamer：潜动态与策略耦合](https://arxiv.org/abs/1912.01603) 与 [JEPA：联合嵌入预测架构](https://arxiv.org/abs/2301.08243)。这些工作共同说明，世界模型真正关心的不是“看起来像世界”，而是“是否保留了足以支持决策和规划的可预测结构”。
...
其中 \(e_\phi\) 是编码器，\(f_\theta\) 是潜动力学，\(h_\psi\) 则是任务头，用于预测奖励、风险、接触事件、终止条件或未来观测摘要。把这一分解写清楚非常重要，因为它解释了为什么“视频预测器”“潜空间动力学模型”和“带奖励头的规划模型”虽然都可能被叫作世界模型，但实际上服务的系统职责并不相同。若只有观测重建头而没有行动相关任务头，它更接近未来观测生成器；若能够稳定输出风险、回报或阶段转移，它才更接近规划可消费的内部世界近似。
...
这一点在阅读文献时尤其重要。某些“视频世界模型”本质上更像生成式未来观察器，某些潜在动力学模型更像策略学习辅助器，某些数字孪生系统虽也预测未来，却主要服务于测试和验证而非内嵌闭环策略。若不先问清一个模型最终服务于控制、评测、数据生成还是内部规划，就很容易把不同问题设定下的结果误放到同一比较坐标系。
...
这也是为什么“看起来同样在做世界模型”的工作，系统地位会非常不同。以 [V-JEPA 2](https://arxiv.org/abs/2506.09985) 为例，其机器人分支 `V-JEPA 2-AC` 不是把模型直接塞进高速控制回路，而是先利用大规模视频预训练形成动作条件世界模型，再用较少机器人视频做后训练，用于图像目标条件下的规划；这说明世界模型在当前更可信的职责之一，是把大规模表征先验转译成较低频的规划与排序能力。另一类工作如 [WorldEval](https://arxiv.org/abs/2505.19017) 则进一步说明，世界模型甚至可以主要服务于“策略评估代理”而非“策略执行器”：其目标不是替机器人下动作，而是在线给不同策略和 checkpoint 做排序、筛除明显危险动作。这类工作特别值得注意，因为它把世界模型的使用位置从“控制之前”扩展到了“研发评估流程内部”。
...
### 47.1 观测空间与潜空间
因此，潜空间设计最需要追问的不是压缩率，而是“保留了哪些任务相关不变量”。对机器人来说，这些不变量往往不是纹理，而是对象身份、相对位姿、接触阶段、可达性关系、风险趋势和动作后果类别。潜变量若无法携带这些量，就算重建得再好，也很难服务规划；潜变量若只对当前任务过拟合，又会损害跨场景复用。这种张力解释了为什么后文很多路线会围绕对象中心潜变量、预测状态表征和任务条件潜变量展开分化。
...
也正因此，很多世界模型论文虽然首先报告一步预测误差，但真正决定系统能否用于规划的，往往是多步 rollout 是否稳定。这个代码片段明确揭示了一件常被忽略的事实：rollout 不是为了“把未来完整演给人看”，而是为了给一组候选动作序列提供可比较的内部评分。
...
在机器人里，这个问题比纯视觉预测更严重，因为系统不会只“看着模型出错”，而会真的按照错误预测去行动。短期偏差可能只影响下一帧重建，但闭环执行会把这种偏差进一步放大，并把机器人推向训练中更少见的新状态分布，随后模型又在这个更偏的分布上继续预测，于是形成典型的“闭环累积误差”。
...
近两年这条路线重新升温，一个重要原因正是大规模视频先验的可用性显著提升。[V-JEPA 2](https://arxiv.org/abs/2506.09985) 展示了“先在超过一百万小时互联网视频上预训练，再用少量机器人轨迹后训练”的路线，这类工作极具吸引力，因为它似乎提供了一条把开放世界视频知识迁移到机器人规划的捷径。对研究者而言，这种路线最诱人的不是“视频更逼真”，而是“也许不必先把对象、接触、动力学全部手工结构化，模型就能自己形成足够有用的未来表征”。

但这条路线的主要反证也越来越清楚。当前很多视频世界模型本质上仍工作在二维序列上，因此对机器人最关键的三维几何、跨视角一致性和物理约束保留不足。[PAIWorld：面向机械臂操作的三维一致性世界基础模型](https://arxiv.org/abs/2606.18375) 直接指出，现有多视角世界模型若只是拼接不同视角的 token，而没有显式几何先验和视角间通信机制，就会出现跨视角对象漂移、深度不一致和纹理错位；[Structured 4D Latent Predictive Model：面向机器人规划的四维结构化潜变量预测模型](https://arxiv.org/abs/2607.01166) 则进一步强调，单纯二维视频序列缺乏精确空间推理所需的三维几何理解。这些工作并不是在否定视频路线，而是在提醒我们：只要下游任务需要精确接触、相对位姿和多视角闭环，纯视频 token 统一接口很容易在最关键的地方失真。
...
对机器人而言，这一路线的吸引力在于它可能更容易保留任务结构，而不是被纹理细节牵着走。
...
更实用的做法，是把这条边界压回三个连续判据。第一，`动作条件性`：模型是否显式依赖动作 \(a_t\)，还是只在被动观察历史上预测未来。若连动作条件都没有，它更接近时间表征学习，而不是机器人意义上的世界模型。第二，`反事实稳定性`：当动作、接近方向或环境约束改变时，内部状态是否会产生结构合理的分支，而不是继续延续训练数据里最常见的视觉惯性。第三，`接口闭合性`：这些潜变量是否真的被下游规划、风险筛选或恢复逻辑消费，而不是只在探针任务上获得漂亮分数。
...
从近两年的工作看，这条边界正在被重新拉实。[V-JEPA 2：面向理解、预测与规划的自监督视频模型](https://arxiv.org/abs/2506.09985) 的机器人分支已经不再满足于“表征好不好”，而是直接把表征推向图像目标条件规划；[WorldEval：把世界模型作为真实机器人策略评估器](https://arxiv.org/abs/2505.19017) 则把世界模型拉进策略排序和危险动作筛除流程；[Structured 4D Latent Predictive Model：面向机器人规划的四维结构化潜变量预测模型](https://arxiv.org/abs/2607.01166) 与 [PAIWorld：面向机械臂操作的三维一致性世界基础模型](https://arxiv.org/abs/2606.18375) 则进一步把几何一致性与机器人规划接口明确写成问题本身。也就是说，当前真正值得高看的一类抽象预测路线，不是“换了个更高级的表征名词”，而是那些开始正面回答“表征如何进入动作后果判断”的工作。
...
如果把这些适用条件写得更工程化，一条相当稳妥的经验法则是：世界模型更适合进入`低频决策层`，而不是`最高频控制层`。也就是说，它更适合用在“每隔若干步做一次候选重排序、未来风险预判、离线策略筛选或数据回放解释”的位置，而不适合成为 100Hz 以上刚性接触控制的唯一闭环中枢。用符号写，就是应尽量满足
...
从当前实证路线看，几类问题尤其适合使用世界模型。第一类是高试错成本、但单次动作后果可通过短视 rollout 近似判断的操作任务，例如目标条件抓取、搬运和局部路径筛选；第二类是训练期样本昂贵、但可通过内部预测提升表示质量或数据利用率的任务；第三类是研发期需要快速比较多个策略版本、却不可能每次都上真机大规模回放的情形，这正是 [WorldEval](https://arxiv.org/abs/2505.19017) 这类工作试图切入的位置。换句话说，世界模型最适合承担“把昂贵真实试错压缩成更廉价内部比较”的职责。

近两年视频 - 潜空间混合路线也强化了这一判断。[V-JEPA 2：面向理解、预测与规划的自监督视频模型](https://arxiv.org/abs/2506.09985) 并未把世界模型直接等同于全栈策略，而是将其用于图像目标条件规划；[Structured 4D Latent Predictive Model：面向机器人规划的四维结构化潜变量预测模型](https://arxiv.org/abs/2607.01166) 与 [PAIWorld：面向机械臂操作的三维一致性世界基础模型](https://arxiv.org/abs/2606.18375) 则强调要把三维结构写回潜变量，才能让预测更可靠地服务规划。这些都说明：当前世界模型最适合的问题，往往不是“直接做出动作”，而是“为动作选择提供更好的内部比较依据”。
...
如果要把“不适合”说得更具体，可以把它们概括成四类高风险任务。第一类是`高带宽硬实时控制`，例如强阻抗接触、双臂精密装配、全身动态平衡；此时模型推演时延和误差传播很容易超过系统容忍度。第二类是`接触模式高度离散且后果陡峭`的任务，例如插接、卡扣、柔性线缆操作；这类任务对摩擦、顺应性和微小姿态误差极敏感，当前大多数世界模型还不足以独立承担最终裁决。第三类是`强责任约束`场景，例如有人共处的工业单元、高价值设备周边操作和需要认证的流程；这些场景要求的不只是平均表现，而是可证明的最坏情况边界。第四类是`长期开放世界自主运行`，即系统必须持续应对新对象、新布局、新人类干预和新异常组合；在这类场景里，内部模型一旦分布外失真，就可能快速积累不可接受风险。

从近期文献看，这些短板并不是“已经被基本解决，只差工程化”那么简单。[PAIWorld：面向机械臂操作的三维一致性世界基础模型](https://arxiv.org/abs/2606.18375) 和 [Structured 4D Latent Predictive Model：面向机器人规划的四维结构化潜变量预测模型](https://arxiv.org/abs/2607.01166) 仍在集中处理跨视角漂移、三维几何一致性与精确空间推理问题；这恰恰说明视频世界模型距离精确接触和空间动作规划还有明显空缺。也就是说，当前研究前沿并没有把这些问题扫清，而是在不断证明“为什么这些缺口不能被轻描淡写地略过”。
...
值得注意的是，规划问题在具身智能里从未消失，只是接口发生了变化。经典规划关注状态空间、动作集合与约束；基础模型时代则把自然语言、常识知识与长程结构化推理重新接入规划器。因此，这一章不应被理解为“旧规划 vs 新模型”的对立，而应被理解为“规划载体和规划接口的迁移”。
这种迁移在语言指导技能调用、代码生成式策略以及开放世界自主代理等路线中都有体现；相关代表性系统可参见 [SayCan：语言指导技能调用](https://arxiv.org/abs/2204.01691)、[Code as Policies：代码生成策略](https://arxiv.org/abs/2209.07753) 与 [Voyager：开放世界自主代理](https://arxiv.org/abs/2305.16291)。
...
这里 `skill` 指向可调用技能，`object_ref` 负责把语言指代绑定到对象状态，`goal_region` 给出目标区域或目标框架，`constraint` 表示姿态容差、禁行区或资源限制，`success` 定义成功条件，`fallback` 则规定失败后回退、重观察或请求帮助的策略。把高层计划写成这种“半结构化契约”的意义，在于它把大模型最容易含糊带过的隐含前提全部显式化。
...
这里 `timeout` 用来避免技能长时间卡住而无人上抛，`verify` 用来规定究竟由视觉、触觉、本体状态还是人类确认来宣布该节点完成。这个补充看似琐碎，实际上极为关键，因为很多“规划失败”并不是路径分解错了，而是节点完成判据和超时机制没有被写进任务结构。
...
行为树在机器人和游戏 AI 中长期流行，并不是因为它最前沿，而是因为它在表达可回退、可条件切换、可并行调度的行为结构上非常实用。对具身系统而言，行为树仍然是把高层任务结构显式化的重要工具，相关综述可参见 [《Behavior Trees in Robotics and AI》](https://arxiv.org/abs/1709.00084)。
...
程序化中间语言的出现，反映的是一种很现实的架构需求：直接从文本到动作太脆弱，而纯符号系统又太僵硬，因此需要一种既保留语义结构、又便于执行验证的中间形式。机器人编排 DSL、技能脚本和可调用接口风格接口都可视为这一路线的不同变体。Code as Policies 的代表性恰恰在于，它把语言模型输出约束到可执行代码接口，使“推理”更容易经过解释器、类型约束和工具调用链路，相关论文可参见 [Code as Policies 论文](https://arxiv.org/abs/2209.07753)。
...
这里 `target_frame` 表示目标位姿或目标区域绑定在哪个坐标系，`tolerance` 规定允许的误差边界，`guard` 表示碰撞、力觉或禁区等守卫条件，`stop` 表示执行完成或应立即停机的判据，`fallback` 则说明失败是局部重试、重观察，还是直接上抛高层重规划。这个结构的关键意义在于，它把“语义目标”翻译成了“控制层知道如何执行、何时停止、何时拒绝执行”的工程对象。
...
其中 `key` 是索引键，`type` 区分环境、任务或交互记忆，\(t_i\) 是写入时间，\(\rho_i\) 表示置信度或来源可靠性，\(\mathrm{ttl}_i\) 表示失效窗口，\(p_i\) 则是负载内容。把记忆显式写成这个结构的意义，在于提醒我们：机器人记忆并不是“多一段上下文文本”，而是一套必须被权限、时效性和来源可信度共同约束的数据基础设施。
...
见：[12-行为树HTN程序化中间表示比较表](docs/report/versions/v0.1/tables/12-行为树HTN程序化中间表示比较表.md)
...
若把不同仿真平台的价值压回到更可执行的比较维度，一个很重要但常被忽略的维度是`接口一致性`而非`视觉保真度`。Gazebo / ROS 生态之所以长期难以被替代，不是因为它最会渲染，而是因为它更容易维持“同一套话题通道、坐标变换、录包回放、控制器管理和状态机”在仿真与真机之间的连续性。对研究团队来说，这意味着仿真阶段暴露的很多问题，本来就是部署阶段最会咬人的那类问题：时钟同步、消息丢失、节点阻塞、坐标系漂移、回放重现和硬件替换后的接口稳定性。

更直观地说，Gazebo / ROS 更像一种“系统同构平台”。它对学习者的真正训练不是“把策略练到最高分”，而是逼你回答一整串平时很容易被论文遮掉的问题：状态估计是如何广播的，感知结果在哪个频率上被消费，控制节点掉线后是否会触发安全停机，回放日志能否跨版本复现同一故障。这也是为什么它即便在高保真合成数据和 GPU 大规模并行方面未必领先，却仍然是很多研究团队进入真实系统之前最关键的组织层训练场。

若把这条生态更抽象地写成研究资产，它最擅长沉淀的并不是“更强策略”，而是四类可复用接口资产：

1. 消息语义：不同模块如何在统一的话题、服务与动作接口上交互。
2. 回放资产：能否用 rosbag 或同类日志重新播放历史故障与边界样本。
3. 替换资产：感知、规划、控制节点能否局部替换而不重写全栈。
4. 硬件在环资产：仿真控制器与真机控制器之间是否能保持相近的组织边界。

也因此，本章更倾向把 Gazebo / ROS 定位成“系统接口与回放资产平台”，而不是与 Isaac Sim、MuJoCo、Habitat 在同一维度上竞争“谁更先进”。它回答的问题不一样：前者更像“我的系统是如何被组织起来的”，后者更像“我的环境与物理是如何被采样与扩展的”。
...
但若继续把“贴近真实”说得太笼统，数字孪生很容易重新滑回美术工程，而不是任务工程。更稳妥的写法，是把场景复刻理解为`任务充分逼真`而不是`全量极致逼真`。若把需要保留的场景变量记为 \(\{q_i\}\)，每个变量对任务成败的敏感度记为 \(w_i\)，则更值得优化的不是整体视觉相似度，而是

\[
\mathcal{F}_{\text{task}}
=
\sum_i w_i \cdot \mathbf{1}[d(q_i^{\text{sim}}, q_i^{\text{real}}) \le \epsilon_i]
\]

这里 \(d(\cdot)\) 表示仿真与现实在某个任务关键变量上的差异，\(\epsilon_i\) 表示该变量的可接受误差阈值。这个式子的意义很直接：货架高度、桌边支撑关系、相机外参、夹具接触面这些高权重变量，往往比地砖纹理、墙面反光或远处装饰物更值得优先复刻。

从近年的 benchmark 看，这种“任务充分逼真”的趋势越来越明显。[BEHAVIOR-1K](https://arxiv.org/abs/2403.09227) 把家庭与日常场景中的 1000 项活动组织成大规模评测集合，其价值不只在任务数量，而在于它迫使场景复刻同时保留对象语义、交互关系与长期任务流程；其官方页面 [BEHAVIOR](https://behavior.stanford.edu/) 也把仿真平台、对象知识和基准任务组织成统一资产。这说明数字孪生真正困难的地方，不是把场景三角面片复制出来，而是把“哪些物体出现、如何交互、在哪些流程节点上被消费”一起复刻。

因此，场景复刻至少应分成四层来理解。第一层是几何层，解决空间拓扑、可达性和碰撞边界；第二层是对象层，解决语义标签、可动物体与接触属性；第三层是观测层，解决相机、深度、遮挡与标定关系；第四层是流程层，解决任务脚本、人工重置与异常插入方式。只有四层共同成立，所谓数字孪生才会真正成为部署前验证资产，而不是漂亮但脆弱的 3D 复制品。
...
如果继续把这条链条写得更完整，观测偏差其实可以看作三种分布同时错位：环境外观分布、传感器响应分布和时间同步分布。很多团队会把注意力放在第一类，例如纹理随机化和光照随机化；但真实部署中第二类与第三类往往同样致命，因为策略真正消费的不是“图片本身”，而是“在某个控制时刻到达系统的观测”。这意味着观测偏差不只是一种视觉问题，而是一个跨感知、时序和决策接口的问题。

也正因此，域随机化并不是“随机得越多越好”。[自动生成抓取数据集的 sim2real 域随机化方法](https://arxiv.org/abs/2310.04517) 代表了一条很典型的思路：通过系统性引入仿真到现实的视觉变化，提升策略跨域鲁棒性。但 [sim2real 之后的安全持续域适配](https://arxiv.org/abs/2503.10949) 也提醒我们，若训练阶段采用过宽的随机化范围，确实可能得到更鲁棒的策略，但也可能同时得到更保守、更低效、对当前真实现场不够贴身的策略；而且部署后环境还会继续漂移，所以仅靠离线一次性随机化并不足够。

因此，一个更成熟的观测偏差治理方案通常至少包括四步：训练期用结构化域随机化覆盖主要变化轴；上线前做传感器标定和时序对齐；运行中记录观测置信与异常触发；上线后允许安全约束下的持续域适配。这样处理后，观测偏差才不会被误解成“图像看着不一样”这种局部问题，而会被纳入一条从仿真外观到现场决策的完整失效链。
...
近年的研究已经开始把这种误差写回更细的参数层，而不再满足于把“摩擦系数”当作一个可随便随机采样的辅助变量。[静摩擦对 sim2real 强化学习迁移的影响](https://arxiv.org/abs/2503.01255) 专门强调静摩擦常常是 sim2real 中被低估的关键误差源，这一点非常重要，因为很多机器人论文讨论接触误差时默认关注的是连续滑动或一般摩擦，而现实中的插接、卡扣、起始抓取与静止接触转动，恰恰首先受静摩擦和接触模式切换支配。

若把这类问题抽象为混合系统，真正困难的不是连续动力学本身，而是模式切换时刻：

\[
\sigma_{t+1} \sim \mathcal{T}(\sigma_t, x_t, a_t, \mu_t, \kappa_t)
\]

其中 \(\sigma_t\) 表示接触模式，\(\mu_t\) 表示局部摩擦条件，\(\kappa_t\) 表示顺应性、间隙或材料属性。只要模式切换时刻被轻微推迟或提前，系统后果就可能从“稳定抓取”跳到“滑移”“卡滞”或“弹开”。这类跳变不是简单靠更长训练时间就能平均掉的，因为它们经常由极小参数变化触发。

因此，对涉及精细操作的仿真结果，更可靠的阅读方式不是问“成功率高不高”，而是问“哪些接触模式已经被显式覆盖，哪些仍依赖隐式幸运拟合”。只要这一步没做，很多高分策略仍然只是“在某组接触先验下有效”，而不是“对接触不确定性真正稳健”。
...
从部署角度看，黑天鹅问题之所以难处理，不只是因为它们稀有，而是因为它们通常表现为`低频 × 高后果 × 多因素耦合`。也就是说，单独看任何一个变量都不极端，但一旦以某种组合方式同时出现，就会迅速越过系统容忍阈值。若把单一异常事件的风险写成

\[
R(e) = p(e) \cdot C(e)
\]

其中 \(p(e)\) 是发生概率，\(C(e)\) 是后果代价，那么黑天鹅管理的难点恰恰在于 \(p(e)\) 可能很低，但 \(C(e)\) 极高，因此靠平均成功率并不能把它们自然“淹没”掉。

这也是为什么成熟团队通常不会只依赖“更多仿真随机化”，而会额外建设三类资产。第一类是`异常场景库`，把历史上真实出现过的极端组合条件沉淀成可回放脚本；第二类是`影子运行与灰度放量`，让新策略先在低权限条件下暴露长尾问题；第三类是`安全持续适配机制`，在不放弃安全边界的前提下吸收部署后新分布。[sim2real 之后的安全持续域适配](https://arxiv.org/abs/2503.10949) 的启发就在这里：sim2real 不是一次性跃迁，而更像受安全约束的长期分布管理问题。

因此，本章更倾向把黑天鹅视为`基础设施成熟度测试`而不是`单个模型缺点`。一个系统能否被长期部署，往往不是看它在平均 episode 上赢了多少，而是看它面对这些低频高代价情形时，是否具备检测、降级、回放、追责和再验证的组织能力。
...
这部分现在也可与 [14-评测指标分层表](docs/report/versions/v0.1/tables/14-评测指标分层表.md) 联动阅读。正文负责解释为什么要分层，表格负责把这些层真正固定成后续复审口径。
...
见 [14-主流仿真平台能力边界表](docs/report/versions/v0.1/tables/14-主流仿真平台能力边界表.md)。
...
如果把 [Jetson Orin](https://www.nvidia.com/en-us/autonomous-machines/embedded-systems/jetson-orin/) 的公开口径进一步拆开看，会更容易理解为什么端侧模型路线一定要重写。NVIDIA 在官方页面里把同代模块明确划成从 `Jetson Orin Nano` 到 `Jetson AGX Orin` 的多个功耗-性能档位，覆盖大致 `7W-60W` 的部署区间与 `67 TOPS-275 TOPS` 的 AI 性能范围。这意味着“同样叫端侧 GPU”在机器人里其实对应完全不同的热设计、供电、机身空间和风道设计条件。对研究者而言，真正有价值的不是背下某个 TOPS 数字，而是意识到：模型结构、上下文长度、输入帧率和动作刷新频率一旦改变，整个热-功耗-时延预算都会被联动改写。
...
从 [ROS 2 官方仓库](https://github.com/ros2/ros2) 的定位也能看出这一点。它并不是单个消息库，而是一组围绕节点通信、消息接口、开发工具和中间件适配共同演化的软件集合。对具身系统而言，这意味着 ROS 2 的真正价值从来不只是“能发消息”，而是它帮团队把哪些状态必须显式暴露、哪些模块必须可回放、哪些节点可以被替换、哪些异常需要被记录，慢慢固定成了一种事实工程文化。后续即便有团队不用原生 ROS 2，也往往还会延续这种“中间件先定义系统边界”的思维方式。
...
这也是 ROS 2、DDS QoS、实时 Linux 与本地 watchdog 机制在具身系统中长期重要的原因。它们并不是“老派工程细节”，而是把概率性 AI 输出装进确定性控制系统的必要胶水，相关系统文档可参见 [ROS 2 文档](https://docs.ros.org/en/rolling/index.html)。

如果把这一点和当前企业公开架构放在一起看，会更清楚为什么“基础模型不可能直接承担全部实时责任”。[Figure Helix](https://www.figure.ai/news/helix) 明确公开了 `S2` 以 `7-9 Hz` 更新高层语义、`S1` 以 `200 Hz` 维持低层控制的分层结构；这与实时系统设计逻辑完全一致，即让语义模型服务于阶段决策与目标更新，而把真正需要硬时间保证的闭环留在更轻、更近、更确定的控制层里。也正因为如此，具身系统里最成熟的路线往往不是“让模型更像控制器”，而是“让控制器与模型之间的责任边界更清楚”。

这个例子很重要，因为它说明“高层模型刷新慢”并不天然意味着系统弱，关键在于刷新慢的那一层到底承担什么责任。若它只负责阶段切换、目标更新和语义解释，那么 `7-9 Hz` 甚至更慢都可能足够；若它试图直接承担接触稳定、姿态保持和轨迹跟踪，那么同样的频率就会明显失效。实时系统设计真正做的，不是追求所有模块都更快，而是按责任把不同模块放进不同的时间预算里。
...
从公开系统看，这种“边缘推理 = 架构重构”已经非常清楚。[Google DeepMind 的端侧版本说明](https://deepmind.google/blog/gemini-robotics-on-device-brings-ai-to-local-robotic-devices/) 强调的是本地低时延和弱联网可运行，而不是单纯公布一个更小模型；[Jetson Orin 官方页面](https://www.nvidia.com/en-us/autonomous-machines/embedded-systems/jetson-orin/) 与 Isaac 生态则把嵌入式 GPU、ROS 加速栈、仿真和部署工具链放在一起讲。这说明边缘部署的主问题已经从“有没有端侧芯片”转成“模型、运行时、调度器和中间件是否被共同设计”。谁若把边缘推理仅理解为导出一个推理引擎，往往会系统性低估具身部署的复杂度。
...
见：[15-硬件约束比较表](docs/report/versions/v0.1/tables/15-硬件约束比较表.md)
...
近年一个很有启发性的变化，是安全 benchmark 开始不再只看“任务做成没有”，而把治理、恢复与升级安全一起拉进评测范围。[EmbodiedGovBench：具身系统治理、恢复与升级安全基准](https://arxiv.org/abs/2604.11174) 就明确把具身智能体的治理、恢复与升级安全作为独立维度来评估。这一点非常重要，因为它意味着红队测试的目标不再只是证明“模型会不会出错”，而是要回答：出错后能否被及时发现，发现后能否进入正确恢复链路，版本升级后这些老问题会不会重新回来。
...
但“人在回路”并不等于“任何时候都让人接管”。更准确的工程问题是：`在哪些节点要求人类批准，哪些节点要求人类监督，哪些节点允许人类直接覆写动作`。若把人类权限强度记为 \(\alpha_t \in [0,1]\)，可以把最简单的共享自治写成
...
这也是为什么近年的共享自治研究越来越强调层级分工与接管接口设计，而不只强调“让人能插手”。[面向关节型机器人的层级规划与策略塑形共享自治](https://arxiv.org/abs/2307.01943) 更接近一种高层任务规划与低层形状约束共同作用的写法；而相邻领域的 [Diffusion-SAFE：安全的人机驾驶接管扩散框架](https://arxiv.org/abs/2505.09889) 则提醒我们，简单动作拼接或朴素交接本身也可能带来新的不安全瞬间。把这些启发迁移到具身系统里，一个更稳健的接管原则是：高层批准、中层监督、低层急停三种权限应当被显式区分，而不是统一丢给“人工可接管”这一句模糊承诺。
...
如果把治理写得更接近当前行业语言，NIST 的表述其实很值得借用。[NIST AI Risk Management Framework 官方页面](https://www.nist.gov/itl/ai-risk-management-framework) 明确把 AI RMF 定义为一套可自愿采用的风险管理资源，用于把“可信性考量”纳入设计、开发、使用和评估。对具身系统来说，这一点尤其关键，因为机器人风险本就跨越设计、训练、部署和运维多个阶段。也就是说，治理不应被理解成上线之后的外部审查，而应被理解成全生命周期中的一套风险分配机制。

因此，责任边界最好不要只写成“谁背锅”，而应写成一张持续运行矩阵。一个更可执行的四角色视图是：

1. 能力声明者：声明模型、本体和技能的已验证边界。
2. 部署批准者：决定哪些边界在当前客户或场景下可被接受。
3. 现场运营者：持有日志、接管、暂停和灰度放量权限。
4. 事故复盘者：在事后界定问题属于能力缺陷、流程缺陷还是边界设定错误。
...
若再往下拆，数据与场景合规至少应被写成四层。第一层是`数据权属层`，关注视频、音频、日志、工艺参数和标注是否有采集与训练授权；第二层是`空间隐私层`，关注环境布局、人员活动轨迹和设备配置是否属于敏感现场知识；第三层是`流程合规层`，关注机器人是否被允许进入某类工位、时间段和操作环节；第四层是`运维日志层`，关注远程诊断、事故回放和模型更新记录是否满足最小必要与可追溯原则。只要其中任一层缺失，系统就可能在“技术上可行、治理上失格”的状态下被错误推进。
...
1. `边界指标`：当前版本允许哪些动作、在哪些工位、以什么速度和权限运行。
2. `接管指标`：接管触发频率、接管延迟、接管后是否进入一致状态。
3. `回归指标`：历史危险样本是否在新版本中重新出现，是否新增未解释空白。
4. `审计指标`：日志是否完整、责任链是否可追溯、版本放行是否有明确签字与依据。

这类面板的重要性，在于它把“治理”从抽象责任讨论，压回可持续维护的数据结构。类似 [EmbodiedGovBench：具身系统治理、恢复与升级安全基准](https://arxiv.org/abs/2604.11174) 这类工作真正提供的启发，也不是某一个具体分数，而是把恢复、治理与升级安全当成可以持续评测的对象。对具身系统而言，只要这些指标长期缺位，所谓治理就很容易退化成“事故之后再开会解释”；只有它们被前置写入日常运转面板，治理才会真正成为系统能力的一部分。
...
在很多公开系统里，最容易泛化的往往是接近抓放（pick-and-place）的结构化任务；而最能暴露能力上限的，通常是需要连续接触调节、局部纠偏和长期稳定性的灵巧任务。ACT、Diffusion Policy、灵巧手策略等路线之所以重要，就在于它们不断逼近这一难题，但还远未彻底解决，相关工作可参见 [ACT](https://arxiv.org/abs/2304.13705) 与 [Diffusion Policy](https://arxiv.org/abs/2303.04137)。
...
近年的工作已经把这一点做得更具体。例如，[arXiv:2606.03949](https://arxiv.org/abs/2606.03949) 不再只把人类反馈当成结果排序，而是把人类干预触发的信息重新用于 credit assignment；[arXiv:2606.32027](https://arxiv.org/abs/2606.32027) 则进一步允许用自然语言定义速度、谨慎程度、稳定性等偏好轴，说明偏好学习正在从“二元选优”走向“多维行为风格约束”。这意味着示教与偏好正在逐渐形成一条连续接口：示教告诉系统“可行解在哪里”，偏好告诉系统“哪类可行解更值得保留”。
...
这一点在经典 shared autonomy 文献里就已经存在。[arXiv:1701.07851](https://arxiv.org/abs/1701.07851) 强调的是系统既要推断人类目标，也要适应人类对系统的信任与可接受性；而更新的 [arXiv:2506.16044](https://arxiv.org/abs/2506.16044) 则明确把人类意图建模、控制混合、责任边界与伦理问题放回统一框架里。再往前一步，[RoboCopilot](https://arxiv.org/abs/2503.07771) 这类工作说明“示教 - 自主 - 再纠偏”可以不是三段式孤立流程，而是一条连续切换的人机共融学习链。
...
端侧路线的重要性在于，它更接近真实部署的时延、隐私和功耗约束。近期无论是 DeepMind 的 Gemini Robotics 路线，还是 Figure 的 Helix 机载双系统结构，都在强调“能部署”本身已经成为研究目标，而不是最终再考虑的工程细节。
更轻量化的 SmolVLA 则进一步说明，部署约束已经开始反过来塑造模型设计目标。
相关资料可参见 [Gemini Robotics 官方介绍](https://deepmind.google/blog/gemini-robotics-brings-ai-into-the-physical-world/)、[Figure Helix 官方介绍](https://www.figure.ai/news/helix) 与 [SmolVLA 论文](https://arxiv.org/abs/2506.01844)。
...
从目前公开的一手材料看，端侧路线已经不再只是概念验证。Google DeepMind 在 [端侧版本官方说明](https://deepmind.google/blog/gemini-robotics-on-device-brings-ai-to-local-robotic-devices/) 中明确把“本地低时延推理、脱离数据网络运行、少量示教快速适配”作为产品化能力来描述，并给出“约 50 到 100 条示教即可适配新任务”的工程口径；Figure 在 [Helix 官方介绍](https://www.figure.ai/news/helix) 中则更进一步公开了异步双系统设计：高层视觉-语言模型 `S2` 以 `7-9 Hz` 运行，低层视觉运动策略 `S1` 以 `200 Hz` 运行，并部署在双嵌入式 GPU 上。它们共同说明，端侧具身模型真正被考验的，不是“能不能跑通一次前向”，而是“能否在物理控制时钟内稳定承担自己的责任层”。
...
其中 \(D_{\mathrm{KL}}(\pi_T \,\|\, \pi_S)\) 不是为了机械逼近教师模型，而是为了尽量继承教师在动作分布、恢复偏好和保守边界上的结构；\(\mathcal{L}_{\text{latency}}\) 与 \(\mathcal{L}_{\text{safety-regret}}\) 则把端侧部署的真实代价显式拉回训练目标。也正因为如此，近年的轻量化路线越来越不是单纯量化或剪枝，而是与动作表示、分层架构和运行时一起共同设计，例如 [SmolVLA](https://arxiv.org/abs/2506.01844) 与 [LeRobot](https://github.com/huggingface/lerobot) 这类社区路线，实际上都把“模型能力”与“低门槛真实部署”并列成了目标，而不是先追最大模型、再在后处理中被动压缩。
...
这也是为什么“低成本平台”不应只被理解为教学玩具。Google DeepMind 在 [端侧版本官方说明](https://deepmind.google/blog/gemini-robotics-on-device-brings-ai-to-local-robotic-devices/) 中明确展示了同一本地化基座模型从 ALOHA 双臂平台继续适配到 Franka FR3 与 Apptronik Apollo 的路线；[LeRobot 开源项目](https://github.com/huggingface/lerobot) 则把“从低成本机械臂 `SO-100` 到人形平台的硬件无关接口”与统一数据格式、统一评测脚本放在同一工具链里。前者说明“轻平台”正在被主流基础模型团队当作适配压力测试，后者说明社区侧也开始把“廉价可复现平台”视作开源生态真正扩散的必要条件。
...
如果结合当前公开系统看，这种架构重组已经是事实而不是建议。[Helix 官方介绍](https://www.figure.ai/news/helix) 把高层语义理解与低层 `200 Hz` 反应控制明确拆开，并让高层异步更新潜变量而不是直接逐控制步覆盖动作；[端侧版本官方说明](https://deepmind.google/blog/gemini-robotics-on-device-brings-ai-to-local-robotic-devices/) 则进一步强调本地低时延与低层安全控制器共存。两者其实在回答同一个问题：高层模型应当在什么频率、以什么形式影响控制栈，才既不浪费语义能力，也不破坏物理稳定性。
...
其中 `Risk` 对应跌倒、碰撞、异常接触、环境损害等风险，`Wear` 对应真实硬件磨损与维护代价。这个表达式的价值在于，它把“探索”重新拉回现实机器人的物理账本: 不是任何新样本都值得拿，而是要在风险预算里选最值得试的样本。

也正因此，长时自主文献经常最终回到“探索如何被约束与审计”。例如，[arXiv:2212.12798](https://arxiv.org/abs/2212.12798) 强调的就不是抽象探索最优性，而是在线适应、环境变化与板载资源约束如何共同塑造长期自主。这种口径与本报告的判断是一致的：探索不是独立子算法，而是持续自主基础设施的一部分。
...
近年的工作已经开始把这条链条显式系统化。例如，[arXiv:2509.23829](https://arxiv.org/abs/2509.23829) 把示教、残差强化学习、滚动执行和数据增广组织成飞轮；[arXiv:2603.11811](https://arxiv.org/abs/2603.11811) 则把语义规划、自动成功评估、环境自动复位和数据路由接起来，说明“自动回流”真正难的不是存日志，而是把运行事件可靠地翻译成高价值训练样本；[arXiv:2511.19647](https://arxiv.org/abs/2511.19647) 更进一步强调部署机器人本身就是数据引擎，而不是训练结束后的静态执行器。三者共同说明：持续自主的关键不在“会不会继续采集数据”，而在“部署、筛选、标注、适配、回放”能否被真正闭环化。
...
1. 算法门槛：是否会探索、会筛样本、会利用新增数据。
2. 系统门槛：是否有日志、回放、版本管理、回退与安全灰度。
3. 组织门槛：是否有人能持续审核、解释和批准能力更新。
...
RoboCup 在今天仍然值得单列，不是因为它只是一个“历史悠久的机器人比赛”，而是因为它实际上构成了一个非常特殊的具身研究共同体：它用长期延续的公开规则、明确的任务边界、对抗或半开放环境、以及高度强调复现与现场表现的制度，把“机器人到底会不会做事”这件事压到可重复比较的场景里。RoboCup 官方一直把长期愿景明确写为“到 2050 年由完全自主的人形机器人击败人类世界杯冠军队”，而其不同 domain 则围绕足球、家庭服务、灾害救援和工业场景分化出不同任务体系。[RoboCup 官方首页](https://www.robocup.org/)、[RoboCup Soccer](https://www.robocup.org/domains/1)、[RoboCup@Home](https://athome.robocup.org/)、[RoboCup Rescue](https://www.robocup.org/domains/2)
...
- 仿真组的核心价值，在于把硬件不确定性暂时拿掉，集中考多智能体决策、协同策略、博弈、队形控制和学习算法。2D/3D Simulation 的长期意义，不是替代真实机器人，而是为高层策略、强化学习和群体协作提供低成本、可大规模试验的平台。[Soccer 2D/3D Simulation League](https://ssim.robocup.org/)
- 标准平台与历史上的 SPL/现行 HSL 标准化分支，更强调“在统一硬件约束下谁能把感知、定位、步态、行为和团队配合做得更稳”。这里的关键不是硬件炫技，而是通过统一平台隔离变量，让算法、软件栈和系统集成能力更可比较。[Humanoid Soccer League](https://hsl.robocup.org/)
- 小型组 SSL 的代表问题则完全不同。它允许高速、小型、自定义机器人，并广泛采用全局相机与场边计算资源，因此更像在考高速多智能体控制、战术协同、轨迹规划和实时决策，而不是像人类那样“每个机器人只靠自己眼睛踢球”。这一路线与人形具身并不等价，但对多机器人协同、实时最优控制和系统工程训练极有价值。[RoboCup Small Size League](https://ssl.robocup.org/)
- 中型组 MSL 则把问题重新拉回到全自主机体：自定义平台、机载感知、实球、实地、高速对抗。这一组更能综合考查机电系统、机载感知、实时定位、团队协作和场上鲁棒性，工程复杂度很高。[RoboCup Middle Size League](https://msl.robocup.org/)
- 人形组如今在官方层面已合并为 [Humanoid Soccer League](https://hsl.robocup.org/)。它最能体现 RoboCup 对“像人一样踢球”这一长期愿景的坚持，因为它要求系统同时面对双足平衡、全身协调、视觉感知、跌倒恢复、踢球动作、攻防协作与规则约束。2026 年 HSL 官方甚至把“首次 11 vs 11 比赛中的正式进球”作为阶段性里程碑公开展示，这说明它真正关心的是把全身自主运动推进到接近真实足球组织的复杂度，而不是只做静态演示。
...
[RoboCup@Home 官方页](https://athome.robocup.org/) 所代表的家庭服务方向，目标最接近家庭与服务机器人：在非标准化家庭环境中，系统要同时面对语音交互、场景理解、物体识别、移动导航、抓取放置、人与机器人协作等问题。它的规则设计长期强调“真实、非脚本化的家庭环境”，这意味着它不是简单做固定流程任务，而是在考“服务场景里的可解释自主性”。近年来，@Home 团队已经开始把 LLM/VLM 接入任务规划与技能调用，例如[Hibikino-Musashi@Home 2024 团队说明文档](https://arxiv.org/abs/2410.06192) 就明确把大语言模型任务规划器与开放技能环境结合起来，这说明该赛道已经开始吸收大模型接口，但仍然必须落回导航、操作与交互的具体系统约束。

Rescue 赛道考查的是灾害场景中的搜救、穿越、感知与任务决策。这里最重要的变量不是“语义多丰富”，而是地形适应、环境不确定性、通信受限、定位困难与任务可靠性。也因此，Rescue 对移动本体、感知鲁棒性、地图构建和人在回路接口的要求非常高。[RoboCup Rescue](https://www.robocup.org/domains/2) 这类赛道提醒我们，很多现实场景中的具身难题首先是“能否在脏乱差环境中活下来”，其次才是“会不会说得更像通用智能体”。

工业方向则把问题进一步拉回到工业物流、仓内执行、装配与智能制造。官方工业域目前包含 RoboCup@Work、物流联盟和智能制造联盟，其共同特征是任务结构更明确、状态可定义性更高，但对节拍、鲁棒性、流程衔接和系统集成的要求极强。[RoboCup Industrial 官方页](https://www.robocup.org/domains/4) 到 2025-2026 年，这一路线的研究焦点还在继续变化。像[智能制造 VLA 基准研究](https://arxiv.org/abs/2507.11402)与[WorkBenchMark 论文](https://arxiv.org/abs/2606.19358) 这类工作，已经开始把工业装配基准、开放词汇感知、规划基线和 VLA 路线直接拉到同一比较框架里，甚至给出了“规划式装配基线优于当前 VLA”这样的结果。这说明工业赛道对今天的大模型路线构成了很有价值的反向压力测试：它逼问我们，语义接口的进步能否真的转化为物理制造场景中的稳定执行。
...
即便到了近年，这种模块化传统仍未消失。[高性能人形足球分层模型系统论文](https://arxiv.org/abs/2512.09431)以及[人形足球快速鲁棒定位论文](https://arxiv.org/abs/2503.11020) 这类工作仍然在持续改进定位、步态、视觉与比赛级系统鲁棒性，说明经典问题并未因为“大模型时代”到来就自动退场。很多比赛中的胜负，今天仍然会被起身速度、定位稳定性、对遮挡的鲁棒感知和恢复逻辑决定。

真正发生变化的，是某些局部能力开始从“纯手工模块”转向“学习增强或学习主导”。例如[SoccerDiffusion 论文](https://arxiv.org/abs/2504.20808)尝试直接从比赛录像学习多模态到关节轨迹的扩散策略，并通过蒸馏把推理压到嵌入式实时部署；[渐进式感知-动作人形足球技能论文](https://arxiv.org/abs/2602.05310)则展示了如何把动作技能学习、轻量感知-动作耦合和考虑物理约束的 sim2real 连成一条渐进式训练路径。它们真正改变的，并不是“足球规则”本身，而是让局部技能获得了比传统手调更强的适应性与数据驱动修正能力。
...
1. 变化最明显的是感知与局部技能层：视觉检测不再完全依赖手工颜色规则，动作生成与步态细节也开始引入示教、扩散策略和仿真蒸馏。
2. 变化次之的是策略组织层：学习方法逐步进入行为选择、局部决策和参数调节，但仍很少替代整套比赛级规则逻辑。
3. 变化最慢的是系统治理层：比赛中的时延治理、跌倒恢复、日志诊断、裁判规则适配、软硬件稳定运行，依然高度依赖传统系统工程。
...
1. 这项能力是否依赖比赛专属外部条件，例如全局视觉、场边计算或特定裁判规则。
2. 这项能力是否仍能在更弱结构化环境中保留价值，例如从固定球场几何迁移到开放作业空间。
3. 这项能力是否对应真实工程中的通用资产，例如状态估计、恢复机制、日志诊断和系统调度。
4. 这项能力是否已经摆脱“围绕比赛得分机制特化”的启发式技巧，转而体现更一般的闭环组织能力。
...
见 [17-前沿专题与系统瓶颈映射表](docs/report/versions/v0.1/tables/17-前沿专题与系统瓶颈映射表.md)。
...
1. [Decision Transformer：回报条件序列决策](https://arxiv.org/abs/2106.01345)。它把离线强化学习重写成“条件于目标回报的序列决策”。
2. [Gato：通用多任务智能体](https://arxiv.org/abs/2205.06175)。它把跨模态多任务智能体压成统一标记流。
3. [RT-1：机器人 Transformer 策略](https://arxiv.org/abs/2212.06817)。它则把这种统一接口进一步落到真实机器人多任务策略学习上。
...
1. 高层模型以什么频率参与决策，而不是持续霸占所有控制周期。
2. 低层闭环如何保留在更快、更窄、更可验证的责任层内。
3. 高质量遥操作数据与实时硬件预算如何共同决定系统边界。

从谱系位置上看，Helix 的关键价值在于：它把高层语义推理与低层快速闭环之间的接口公开讲清楚了。这使它成为观察“高低频分层是否会成为产业主流组织方式”的重要样本，而不只是又一个企业演示案例。

### 89.4 Physical Intelligence π0 路线
...
本章后续维护时，建议直接配合 [18-论文谱系字段表](docs/report/versions/v0.1/tables/18-论文谱系字段表.md) 使用。先填字段，再写评价，比先写印象式评论更能保持跨版本口径稳定。
...
这里的 `source type` 回答数据来自真机、遥操作、仿真还是互联网；`interaction level` 回答数据是被动观察还是带真实交互后果；`organization mode` 回答这些数据是单平台专用、跨平台对齐、带失败样本回流，还是经过强人工清洗；`failure coverage` 回答数据是否包含偏差与恢复；`rights status` 则提醒后续维护者不要忽略再训练与再分发边界。
...
这样即便不同论文都使用了同一个 `generalization` 术语，我们也仍能看清它们到底是在同一维度上竞争，还是只是在共享一个容易被高估的口号。比如，“换了新杯子仍能抓取”与“换了新机器人本体仍能执行同类任务”都可能被写成泛化，但它们对系统设计的要求完全不同。
...
这也是为什么泛化目标列在比较时几乎应先于总成功率。若一条路线只在 `object` 维度外推，而另一条路线尝试在 `environment` 与 `recovery/time` 同时外推，那么二者的成功率数字本来就不应直接并列。很多被媒体或社交网络渲染成“谁更通用”的争论，本质上只是没有先把泛化变量写清楚。
...
2. `L2`：提供权重或演示接口。
3. `L3`：提供训练/推理代码。
4. `L4`：连同数据协议、评测脚本和环境配置一并开放。
5. `L5`：形成持续维护、社区反馈和二次开发生态。
...
这一字段还会直接影响报告后续的研究资产沉淀方式。对于 `L1-L2` 路线，更适合保留为论文卡和路线级判断；对于 `L3-L4` 路线，则值得进一步沉淀为可复现的工具链、配置记录和对比表；而只有接近 `L5` 的路线，才真正值得在长期版本里持续当作公共基础设施跟踪。也就是说，开源程度不只是方法学评价项，还会决定本项目后续该把多少维护预算投入到对应路线。
...
在当前版本中，`图 18-1 论文谱系时间线图` 已承担主时间线职责；`表 18-1 论文谱系字段表` 与 [18-论文谱系时间线表](docs/report/versions/v0.1/tables/18-论文谱系时间线表.md) 则共同承担结构化记录与时间轴维护职责。
...
如果后续要做更严格的增量维护，本章建议优先同时配合两张结构化表来读：一张是字段表 [18-论文谱系字段表](docs/report/versions/v0.1/tables/18-论文谱系字段表.md)，用于固定“怎么记录一篇论文”；另一张是 [18-论文谱系时间线表](docs/report/versions/v0.1/tables/18-论文谱系时间线表.md)，用于固定“怎么把论文放回年份与主线里”。前者解决记录口径，后者解决时间结构；两者合在一起，才能真正支撑后续季度增量更新。
...
见：[18-论文谱系字段表](docs/report/versions/v0.1/tables/18-论文谱系字段表.md)

## 表 18-2 论文谱系时间线表

见：[18-论文谱系时间线表](docs/report/versions/v0.1/tables/18-论文谱系时间线表.md)
...
我更建议把“机器人基础模型开源度”再拆成三层来看。第一层是论文级开放，只告诉你模型大致结构；第二层是代码/权重级开放，让你能真正复现实验；第三层是协议级开放，把数据 schema、动作表示、评测脚本、适配方法和部署入口一起公开。只有到第三层，项目才更接近可继承基础设施。像 [OpenVLA GitHub](https://github.com/openvla/openvla) 这种同时公开模型、数据接口、微调与服务化入口的项目，和只给 checkpoint 或演示脚本的项目，研究价值并不在同一个数量级上。
...
这些工作为更通用的策略学习提供了基线，使动作生成、轨迹建模和条件化策略学习得以进入更统一的比较框架。代表性论文可参见 [Diffusion Policy：扩散式机器人策略](https://arxiv.org/abs/2303.04137)、[Decision Transformer：序列化决策](https://arxiv.org/abs/2106.01345) 与 [ACT：动作块 Transformer](https://arxiv.org/abs/2304.13705)。

如果把这一类开源 policy 按“动作是怎样被组织出来的”来分层，至少可以得到四类主流写法。第一类是`自回归序列策略`，其典型形式更接近

\[
p(a_t \mid o_{\le t}, a_{<t}, g)
\]

即把决策写成条件序列建模问题，[Decision Transformer：序列化决策模型](https://arxiv.org/abs/2106.01345) 是这一思想在决策领域的经典入口。第二类是`动作块策略`，直接预测未来 \(H\) 步动作片段，

\[
p(a_{t:t+H-1} \mid o_{\le t}, g)
\]

这样做的好处是减少逐步解码抖动，[ACT：动作块 Transformer](https://arxiv.org/abs/2304.13705) 就把“动作块”明确写成策略接口。第三类是`扩散式动作策略`，以迭代去噪方式生成动作序列，[Diffusion Policy：扩散式机器人策略](https://arxiv.org/abs/2303.04137) 代表了这种路线，其优势在于对多峰动作分布的刻画能力更强。第四类则是`通用策略底座 + 跨本体数据协议`路线，例如 [Octo：开源通用机器人策略](https://arxiv.org/abs/2405.12213)、[OpenVLA：开放视觉-语言-动作模型](https://arxiv.org/abs/2406.09246) 与 [Open X-Embodiment：跨本体机器人数据开放协议](https://arxiv.org/abs/2310.08864) 所共同推动的方向：不再只优化单一本体上的策略拟合，而是把动作接口、数据模式与迁移方式一起做成生态对象。

因此，读一个开源策略项目时，至少应先看四个维度。第一是`动作表示`：到底输出单步连续动作、动作块、离散动作记号还是技能索引。第二是`时间抽象`：它是在毫秒级控制上出动作，还是在较低频率上出未来一段动作计划。第三是`跨本体适配方式`：它假设所有机器人共享同一动作语义，还是允许通过适配器、归一化和数据模式映射来组织差异。第四是`运行时接口`：模型训练出来之后，是只停留在笔记本实验或离线展开脚本，还是已经暴露推理服务、机器人适配、日志和回放接口。只要这四维被看清，一个策略仓库的研究位置就会比“代码托管平台热度高不高”更明确。

从这个角度看，policy 开源项目还可以进一步分成三类资产。`研究样本`主要公开论文对应实现，用于说明方法是否成立；`系统样本`开始把数据处理、评测和部署入口一起暴露出来，用于说明方法如何接进真实软件栈；`基础设施样本`则进一步把数据 schema、训练脚本、机器人适配、回放和运行时服务都稳定公开，使后来者可以在其上持续叠加。像 [OpenVLA GitHub](https://github.com/openvla/openvla) 与 [LeRobot GitHub](https://github.com/huggingface/lerobot) 之所以值得长期跟踪，正是因为它们开始显式提供第二类甚至向第三类过渡的资产，而不只是“模型权重下载入口”。

同样重要的是，要把失败模式一起写进阅读框架。当前开源 policy 最常见的四类脆弱点是：第一，`恢复盲区`，即模型擅长主路径模仿，却缺少失败后回退与二次尝试能力；第二，`动作接口错配`，即离线动作格式与真实控制器消费接口不一致；第三，`时延敏感性`，即模型在离线 rollout 中表现良好，但在边缘设备或网络链路上推理抖动明显；第四，`跨本体幻觉`，即表面上支持多机器人训练，实际上控制频率、夹具和示教语义并未真正对齐。只要这些问题没有被公开处理，再漂亮的开源 policy 也更接近“研究现象样本”而不是“可继承工程资产”。

对后续版本维护而言，这一节最值得固定成纪律的不是项目名单，而是一条阅读顺序：

1. 先看数据协议：这个 policy 默认读取什么 schema、什么时间切片、什么动作语义。
2. 再看动作表示：它输出单步、chunk、token 还是技能。
3. 再看评测协议：它的 benchmark 到底奖励主路径命中、恢复能力还是长时稳定性。
4. 最后看运行时接口：它是否真正暴露服务化、日志、适配和回退入口。
...
这一层近两年最值得重视的变化，是轻量化项目不再只发布“压缩版模型”，而开始把低门槛硬件、统一数据格式和最小复现实验一起交给社区。[LeRobot](https://github.com/huggingface/lerobot) 明确把真实机器人训练、评测、遥操作、低成本机械臂与硬件无关接口放进同一项目叙事；[SmolVLA](https://arxiv.org/abs/2506.01844) 则把“小模型也要保留真实机器人可用性”作为正式研究目标。这说明端侧开源路线的价值，已经从“给弱设备一个替代品”升级成“把真实部署约束重新写回研究默认设定”。
...
Open X-Embodiment 的重要性在于它尝试把跨平台机器人数据组织成可比较接口，这是机器人数据工程走向基础模型的关键一步，相关论文可参见 [《Open X-Embodiment》](https://arxiv.org/abs/2310.08864)。
...
导航和具身智能研究社区长期积累了大量场景级数据集与评测基准，为长时程任务、场景记忆和环境交互研究提供了可复用基础。Habitat 等环境也让“导航评测并不是单个数据集，而是一套可反复实例化的实验接口”这一点更清楚，相关资料可参见 [Habitat 项目主页](https://aihabitat.org/)。

### 92.3 多机器人 / 多模态数据集
多机器人 / 多模态数据集的真正价值，不只是“样本量更大”，而是它们尝试统一观测、动作、时间戳、元数据与任务描述接口，使不同本体、不同传感器组合、不同采集流程之间第一次出现最基本的互操作可能。Open X-Embodiment 的启发也在这里：如果行业长期停留在各家自定义数据模式、各家自定义 episode 格式，就很难形成可持续的跨平台学习生态，相关论文可参见 [《Open X-Embodiment》](https://arxiv.org/abs/2310.08864)。
...
这一点与 [14-评测指标分层表](docs/report/versions/v0.1/tables/14-评测指标分层表.md) 可以互相配合。前者帮助判断 benchmark 奖励什么，后者帮助判断这些奖励是否足以支撑真实部署判断。
...
Isaac、Habitat、MuJoCo、ROS/Gazebo 等平台构成了机器人研究最重要的工具链底座。问题不在“哪个最好”，而在不同工具链分别服务于什么能力边界；相关入口可参见 [Isaac Sim](https://developer.nvidia.com/isaac/sim)、[Habitat](https://aihabitat.org/)、[MuJoCo](https://mujoco.org/) 与 [Gazebo](https://gazebosim.org/home)。

如果再往前走一步，我会建议把“仿真工具”与“仿真训练框架”分开看。前者更接近物理与渲染底座，后者则决定如何在这些底座上批量生成任务、并行训练与做回归评测。[Isaac Lab](https://github.com/isaac-sim/IsaacLab) 的价值，就不只是另一个仿真器，而是把“大规模并行机器人学习”组织成了更可重用的训练框架；[Habitat](https://aihabitat.org/) 则更长期地承担了 embodied AI 环境、场景任务与评测接口的统一入口。对学习者而言，这种区分很重要，因为很多时候真正缺的不是“再多一个物理引擎”，而是“把训练和评测规模化组织起来的脚手架”。
...
从这个标准出发，当前最值得重视的开源变化，是越来越多项目开始公开“训练之后怎么办”。例如 [OpenVLA GitHub](https://github.com/openvla/openvla) 已经把推理服务、机器人适配和后续微调入口显式暴露出来；[LeRobot](https://github.com/huggingface/lerobot) 则把数据采集、真实机器人评测、模型训练和部署链路组织成一套更完整的研究工作台。它们未必已经解决了全部部署问题，但至少把社区最容易缺失的那一层接口第一次变得足够可见了。
...
在当前版本中，`图 19-1 开源生态四层结构图` 已承担分层结构说明；[19-代表性开源项目对照表](docs/report/versions/v0.1/tables/19-代表性开源项目对照表.md) 与 `表 19-1 开源生态四层对照表` 则分别负责“项目级横向对照”与“生态层级定位”。
...
目前这章已经可以配合 [19-开源生态四层对照表](docs/report/versions/v0.1/tables/19-开源生态四层对照表.md) 和 [19-代表性开源项目对照表](docs/report/versions/v0.1/tables/19-代表性开源项目对照表.md) 一起读。前者负责回答“开源生态分几层”，后者负责回答“具体项目在这几层中的什么位置、适合拿来做什么”。这样后续补项目时，就不容易重新退化成仓库名单。

## 表 19-1 开源生态四层对照表

见 [19-开源生态四层对照表](docs/report/versions/v0.1/tables/19-开源生态四层对照表.md)。

## 表 19-2 代表性开源项目对照表

见 [19-代表性开源项目对照表](docs/report/versions/v0.1/tables/19-代表性开源项目对照表.md)。

从维护方法上看，后续新增开源项目时，建议先查 [开源生态与工具链论文清单](D:/Projects/embodied-intelligence-report/research/papers/开源生态与工具链-论文清单-v0.0.md)，再判断是否需要进入 [19-代表性开源项目对照表](docs/report/versions/v0.1/tables/19-代表性开源项目对照表.md) 或 `data/processed/开源生态分层-v0.0.csv`。
...
目前这套模板已经落成表格版本，见 [20-企业统一分析模板](docs/report/versions/v0.1/tables/20-企业统一分析模板.md)。后续版本若要做季度更新，可直接配合 [20-企业季度跟踪表模板](docs/report/versions/v0.1/tables/20-企业季度跟踪表模板.md) 使用，把横向比较进一步转成可追踪字段。
...
这五类路线并不互斥，但通常每家公司只有一类主轴最强。真正高质量的路线判断，不是简单说“它几类都做”，而是识别哪一类是其现金流、组织能力和技术叙事真正围绕的中心。以 Figure 在 2025 年发布的 Helix 为例，其官方说明强调的是通用 VLA、双系统分层、低功耗端侧部署和高频全上肢控制，这说明它当时的主轴不是单纯“做人形”，而是“以统一模型接口重写动作组织方式”，同时保留高低频分层这一工程妥协。[Figure Helix 官方说明](https://www.figure.ai/news/helix)
...
1. 真实部署频率，而不是单次展示频率。它回答的是系统是否真的在现场被反复使用。
2. 失败恢复链路，而不是平均成功率。它回答的是系统出错后能否回到可运营状态。
3. 本体迭代节奏，而不是只看模型迭代节奏。它回答的是硬件路线是否在收敛。
4. 客户约束强度，而不是泛泛“行业合作”。它回答的是场景是否对应高价值、强需求、可验收的问题。
5. 供应链与维护能力，而不是单纯融资速度。它回答的是公司能否从 prototype 过渡到 product。
...
这一点其实已经能从当前较成熟的工业与巡检产品页面里看到。以优傲机器人（Universal Robots）为例，其官方产品页强调的是协作机械臂的安全、灵活、易部署、易编程和可扩展性，并进一步区分不同系列在性能、可靠性和服务体系上的差异。这种表述的中心不是一次演示动作，而是部署、维护和复制能力。[优傲机器人官方产品页](https://www.universal-robots.com/products/) 同样地，波士顿动力（Boston Dynamics）对 Spot 的介绍重点放在动态感知、工业巡检、危险环境进入、自动充电、动态重规划、自扶正和企业级部署支持上。这些字段都更接近“交付友好指标”，而不是“演示友好指标”。[波士顿动力 Spot 官方产品页](https://bostondynamics.com/products/spot/) 这也说明，真正值得长期记录的，往往是系统如何被持续交付，而不是它曾经做出过多精彩的一次动作。
...
更稳健的做法，是再给每个分量附一个证据等级 \(e \in \{\text{story}, \text{demo}, \text{pilot}, \text{deployment}, \text{operations}\}\)。同样是“交付能力强”，如果证据只停留在公开视频，和已经有跨站点连续运行记录，研究判断的权重完全不应相同。也就是说，本章真正想固定下来的，不是公司名次，而是“证据先于叙事”的比较纪律。
...
见 [20-企业统一分析模板](docs/report/versions/v0.1/tables/20-企业统一分析模板.md)。
...
“同一企业多轮版本更新如何回写判断层”这一问题，本质上要求报告具备跨时间的结构化记忆能力。单靠正文叙述很难稳定维护，因此更适合通过 [20-企业季度跟踪表模板](docs/report/versions/v0.1/tables/20-企业季度跟踪表模板.md) 与企业长期跟踪卡来承载。正文负责输出阶段性判断，结构化资产负责记录新增信号、口径变化和判断回调依据，两者结合后，企业研究才不会退化成一次性的公司简介。

为了让这一章真正变成后续维护的工作台，而不是只作为阅读框架存在，建议后续企业更新统一依赖两张表：一张是 [20-企业统一分析模板](docs/report/versions/v0.1/tables/20-企业统一分析模板.md)，负责稳定横向比较维度；另一张是 [20-企业季度跟踪表模板](docs/report/versions/v0.1/tables/20-企业季度跟踪表模板.md)，负责记录季度新增信号与是否需要回调专题。这样做后，企业章节的更新路径就会从“重写公司简介”转变成“先补信号，再修判断”。

如果需要进一步降低后续维护成本，也可以直接优先使用脚本导出版：[20-企业统一分析模板-脚本生成版](docs/report/versions/v0.1/tables/20-企业统一分析模板-脚本生成版.md) 与 [20-企业季度跟踪表模板-脚本生成版](docs/report/versions/v0.1/tables/20-企业季度跟踪表模板-脚本生成版.md)。其底层字段源位于 `data/processed/`，便于后续先改结构化字段，再统一导出到正文表格。
...
如果后续要把季度信号沉淀到统一结构化表中，则应同步更新 [20-企业季度跟踪表模板-脚本生成版](docs/report/versions/v0.1/tables/20-企业季度跟踪表模板-脚本生成版.md) 对应的底层 csv，而不是只在正文里补一句“本季度有变化”。
...
见 [21-海外重点企业路线对照表](docs/report/versions/v0.1/tables/21-海外重点企业路线对照表.md)。
...
目前这一章已经可以先配合 [22-国内重点企业路线对照表](docs/report/versions/v0.1/tables/22-国内重点企业路线对照表.md) 一起使用。正文负责解释为什么国内企业不能只按海外叙事镜像比较，表格则负责把“本体、场景、模型来源、制造、交付、风险”这些维度稳定下来，供后续季度更新直接复用。

## 表 22-1 国内重点企业路线对照表

见 [22-国内重点企业路线对照表](docs/report/versions/v0.1/tables/22-国内重点企业路线对照表.md)。
...
更进一步说，RaaS 真正售卖的是一套可度量的服务纪律，而不是一台机器的月租。只要企业不能稳定回答“在线率是多少、平均恢复时间是多少、谁负责升级窗口、跨客户复制时额外定制化增加多少”，它就还没有形成真正意义上的 RaaS。换言之，RaaS 的本质不是金融包装，而是把机器人系统强制推入 `SLA` 思维。

因此，判断 RaaS 时最值得问的，不是“这个模式听起来先进不先进”，而是企业是否已经形成可度量的 `SLA` 文化。能否承诺在线率、恢复时间、升级窗口和责任边界；是否具备日志回放、远程维护、故障分级与备件网络；是否能控制场景定制化蔓延，这些都比销售话术更能说明模式是否真的成立。

从失败模式看，RaaS 最常见的风险通常有四类：设备利用率不足、维护成本失控、客户定制化吞噬标准化、资产负债表承受持续扩张压力。更细一点说，后续最该长期记录的是四个运营字段：`uptime`、`MTTR`、`operator-to-robot ratio`、`gross margin after service`。只要这些字段中有两三项持续恶化，RaaS 就会从“降低客户门槛”的优势，迅速转化为“企业自己背运维与财务压力”的负担。
...
1. `demo success`：技术可行，但尚未形成客户与运维闭环。
2. `pilot success`：已有真实客户试点，但复制性与单位经济性仍未站稳。
3. `business success`：价值、维护、责任与复制四者同时闭环。
...
见 [23-典型场景任务单元对照表](docs/report/versions/v0.1/tables/23-典型场景任务单元对照表.md)。
...
“值不值得先做”的问题，最终需要被转译成半结构化的 ROI 判断，而不是只停留在定性描述上。[23-场景ROI粗估模板](docs/report/versions/v0.1/tables/23-场景ROI粗估模板.md) 的作用，正是在“人工成本替代、部署改造成本、维护负担、责任风险和复制难度”之间建立统一口径，使场景优先级讨论具备跨版本可更新性。

后续如果要把这一章真正用作季度判断工具，建议同时配合 [23-典型场景任务单元对照表](docs/report/versions/v0.1/tables/23-典型场景任务单元对照表.md) 和 [23-场景ROI粗估模板](docs/report/versions/v0.1/tables/23-场景ROI粗估模板.md) 使用。前者负责拆任务单元，后者负责把“值不值得先做”写成统一粗估口径。

若后续更偏向结构化维护，则可以优先使用 [23-场景ROI粗估模板-脚本生成版](docs/report/versions/v0.1/tables/23-场景ROI粗估模板-脚本生成版.md)。这样场景字段可以先在 `data/processed/场景ROI粗估模板-v0.0.csv` 中修改，再统一导出，避免正文表格和底层字段长期分叉。
...
见 [24-产业信号分类表](docs/report/versions/v0.1/tables/24-产业信号分类表.md)。
...
为了让这章真正服务后续更新，而不只是提供分析口径，建议后续产业跟踪统一同时维护 [24-产业信号分类表](docs/report/versions/v0.1/tables/24-产业信号分类表.md) 和 [24-产业信号日志模板](docs/report/versions/v0.1/tables/24-产业信号日志模板.md)。前者负责分类语言，后者负责把新增事件按“时间 - 类型 - 影响层 - 影响章节 - 判断动作”记录下来，供季度回调直接使用。
...
这样一来，持续跟踪就不再是资料收集工作，而会变成版本治理工作。论文可配合 [18-论文谱系字段表](docs/report/versions/v0.1/tables/18-论文谱系字段表.md)，企业可配合 [20-企业季度跟踪表模板](docs/report/versions/v0.1/tables/20-企业季度跟踪表模板.md)，任务失败与恢复可配合 [17-任务恢复模式表](docs/report/versions/v0.1/tables/17-任务恢复模式表.md)。正文负责沉淀较稳定判断，卡片与日志负责承接高频信号，两者之间用“是否改写比较口径”来决定是否回写。
...
我当前更愿意把它们明确写成五条“中期基线判断 + 潜在失效条件”：

1. 具身系统的真正瓶颈仍然是闭环，而不是单次感知或单次推理。凡是只提高离线理解、单步演示成功率，却没有显著改善接触、恢复、异常处理和长时运行稳定性的进展，都不足以推翻这条判断。
2. 大模型带来的最大变化是高层接口重组，而不是自动消灭物理世界约束。VLA、统一多模态接口和语言条件化策略很重要，但它们只有在动作接口、数据闭环和部署链路中真正站稳，才代表能力边界发生了改变。[OpenVLA](https://arxiv.org/abs/2406.09246)、[RT-2](https://arxiv.org/abs/2307.15818)
3. 数据工程、评测协议和恢复机制在机器人里常常与模型本身同等重要。若一条路线无法回答数据从哪里来、失败如何回流、能力如何被评测和回放，那么它即使模型叙事很强，也更接近展示型系统。
4. 世界模型、技能层和大小脑分层并不是旁支，而是解决统一模型现实落地难题的关键补充。世界模型与抽象预测的价值，不在于“看起来更像大脑”，而在于是否真正进入决策链、规划链和恢复链。[Dreamer](https://arxiv.org/abs/1912.01603)、[JEPA](https://arxiv.org/abs/2301.08243)
5. 真实能力边界更多由数据、状态估计、硬件和系统工程定义，而不只是由模型大小定义。端侧约束、系统安全与部署摩擦会持续反向塑造模型形态；若这些约束没有被显著穿透，单纯增加参数或训练规模并不足以改写系统位置。[Open X-Embodiment](https://arxiv.org/abs/2310.08864)、[Figure Helix](https://www.figure.ai/news/helix)

这五个判断之所以值得保留，是因为它们都不是由某一篇单独论文得出，而是来自对多个章节共同约束的归纳。对后续版本维护来说，更关键的问题不是“又出现了什么新名词”，而是它究竟强化了哪一条基线，还是已经开始动摇哪一条基线。只要这个问题先被问清，结论章就不会随着热点切换而大幅摇摆。
...
benchmark 与数据集变化应被视为路线变化信号，而不只是附录更新。新的操作数据集、跨本体 schema、仿真评测协议或恢复能力指标，一旦进入社区共识，就可能比一篇单独的新模型论文更能改写后续研究方向。谁定义了常用数据 schema、评测协议和成功指标，谁就在一定程度上定义了什么被算作“进步”。

因此，后续版本维护时，建议把每个重要 benchmark 或数据集都压成统一字段：

`BenchmarkRecord = (Task Unit, Morphology, Action/State Schema, Success Definition, Recovery Metric, Data Source, Adoption Scope, Affected Chapters)`

这个记录格式最重要的地方，不是把信息记全，而是强迫每一个 benchmark 都回答三类问题：

1. 它鼓励什么能力，又忽略什么能力。
2. 它是否正在成为事实标准，还是仍局限于某一社区小圈子。
3. 它一旦被广泛采用，会回调哪些章节的比较口径。

从长期跟踪角度看，至少应把这类对象分组维护：

1. 跨本体数据集与统一 schema，关注它们是否推动数据可混合、可复用、可迁移。
2. 移动操作与多阶段任务 benchmark，关注它们是否真正测到恢复、重规划和长时执行。
3. 世界模型、长时程控制与反事实评测协议，关注它们是否把“预测好看”变成“决策有用”。
4. 仿真到现实和域迁移评测，关注它们是否改变部署前验证方法。
5. 恢复、异常处理与人机接管指标，关注它们是否开始进入主流评测而不再只看成功率。

对后续版本维护来说，这一栏还是直接的章节回调触发器。只要 benchmark 口径变了，许多旧判断的比较基础就会跟着变化。因此，不应只记录 benchmark 名称和发布日期，还应明确绑定它影响的是哪些章节、哪些结论和哪些比较口径。只有把“被影响对象”一并写清楚，这一栏才真正具有回调正文和回调大纲的能力，而不是停留在资料清单层。
...
季度更新真正承担的是“修判断”的职责。与月度更新不同，它不只是收集信息，而是要回头检查哪些中期判断被强化、哪些开始松动、哪些章节需要增修或重构。也就是说，季度栏目是版本演进的真正控制点。
...
3. 哪些章节需要补引用、补案例、补图表？
4. 哪些章节明确无需改动？
5. 哪些章节已出现结构性失衡，需要回调大纲？
```

只要按这个顺序做，季度更新就不容易沦为“把三个月新闻拼成一篇摘要”，而会真正承担起整本报告的判断校准功能。
...
这三分法很重要，因为研究型维护不应只会追加内容，也必须敢于下调某些章节中的表述强度、代表性案例位置和争议权重。若季度复盘发现多个章节的前提条件都发生了联动变化，例如 benchmark 口径变化、关键平台接口变化和企业交付证据变化同时出现，那么季度更新就不应停留在修段落层，而应升级为局部结构复审，必要时同步回调大纲。
...
综合来看，一个较稳妥的更新节律是：月度收素材，季度复判断，重大事件做跨章节回调，半年或年度做结构复审。这样既能保证报告跟得上变化，又不会因为追逐每一个热点而把研究型结构打散。

这套节律与本项目的目录结构、论文卡、企业卡和表格化资产是相互配套的。后续只要继续沿这套节律维护，`current` 就能持续吸收新证据，而冻结版本只在关键节点生成。也因此，后续版本维护最需要避免的，不是“更新不够快”，而是“所有变化都直接写进正文”。正文应只吸收那些已经足以改变章节级判断的证据；尚不足以改写结论、但值得保留的动态，更适合先沉淀进 `research/`、长期跟踪卡、季度更新表或结构化表格。

从执行上看，我建议把更新动作明确分成三类：

1. 日志型更新：只补充事实，不改正文判断。
2. 判断型更新：在章节内修正结论、对比口径或争议权重。
3. 结构型更新：当多个章节的边界同时失真时，再回到 `docs/planning/` 调整大纲与章节职责。

只要这三类动作先被区分，后续版本就能做到“增量资料很多，但结构改动克制且有据可依”。为了让报告长期可维护，建议至少遵循以下四层节律：
...
也就是说，月更主要回答“发生了什么”，季更主要回答“哪些判断需要调整”，重大事件更新回答“是否出现不能等到固定周期的结构变化”，而半年/年更则回答“这份报告的分析框架本身是否也要升级”。

为了把这套节律真正转成长期可执行动作，建议后续更新时直接配合 [26-版本更新节律与动作模板](docs/report/versions/v0.1/tables/26-版本更新节律与动作模板.md) 使用。正文负责解释为什么要区分月更、季更、半年/年更与重大事件更新，表格负责把每种节律的目标、记录对象、是否回写正文和典型触发条件固定下来。只有把这些动作模板写实，版本维护才不会重新退化成凭感觉更新。

从长期维护方法上看，最重要的其实不是“更快更新”，而是让每一类更新都有清晰职责边界。只要职责边界稳定，哪怕未来换人、换模型、换工作窗口，这套报告也仍然可以沿同一条研究工作流继续长出来，而不会重新退化成一次性资料堆积。
...
更进一步说，本章承担的是“主线章节之间的回调缓冲层”职责。像 RoboCup 这样的话题，会同时牵引 `05-控制与状态估计`、`06-学习闭环`、`12-规划与决策`、`14-评测基础设施` 与 `17-学术前沿`；多模态编码逻辑则会同时牵引 `07-大模型基础`、`08-系统架构与分层接口`、`11-VLA` 与 `13-数据工程`。若把这些内容平均摊回各章，读者看到的只会是若干分散片段；而把它们保留在同一专题里，反而更容易看清它究竟如何跨章节地改变判断口径。
...
其中 `cross-chapter pull` 表示该议题是否稳定牵引多个主线章节，`evidence flow` 表示近一到两年是否有持续、公开、可复查的新证据流，`judgment impact` 表示它是否足以回调全书的既有判断。只有这三个量至少有两个持续为高，本章中的专题位置才是合理的；否则，它更适合降级到 `research/` 下的研究资产，而不必长期占据正式正文。
...
1. 正文专题：位于本章，用于陈述阶段性判断和与主线的关系。
2. 研究底稿：位于 `research/notes/`，用于沉淀资料清单、问题定义、路线分歧与下一轮研究入口。
3. 结构化卡片：位于 `research/papers/`、`research/companies/` 或后续新增的规则/benchmark 目录中，用于长期积累一手证据。
4. 回调记录：位于 `docs/planning/current/当前工作清单.md` 与 `docs/planning/current/当前版本修订计划.md`，用于记录“何时因该专题触发正文回写”。

例如，RoboCup 专题不应只在本章存在，还应同时挂接 [27-兴趣议题编录与延伸研究-研究底稿](D:/Projects/embodied-intelligence-report/research/notes/27-兴趣议题编录与延伸研究-研究底稿.md) 中的赛道与论文清单；多模态编码逻辑专题则不应只停留在正文论证里，还应持续回写到相关论文卡、实验问题清单与 `07/08/11/13` 的技术判断上。只有这样，专题才不会变成一次性长文，而会真正变成跨版本可维护的知识索引。
...
1. 先记录新增证据：论文、规则变化、官方技术文档、工程案例或 benchmark 更新。
2. 再判断影响范围：它是否只改变专题内部判断，还是已经触发 `>= 2` 个主线章节的判断变化。
3. 最后决定回写层级：只更新 `research/` 资产、回写本章专题，还是同步回写主线章节与工作清单。
...
1. `Soccer` 赛道覆盖标准平台、仿真、多足/轮式中型平台、人形等不同本体约束，难点从视觉定位、队形协作、嵌入式实时通信一路延伸到步态控制、摔倒恢复和高速对抗下的局部重规划。
2. `@Home` 更像“家庭服务系统测试场”，常用移动底盘、机械臂、麦克风阵列和语音/视觉交互模块，重点考查任务分解、语义理解、导航与服务流程组织。
3. `Rescue` 把不确定地形、受限感知、鲁棒通信和人机协同放在前台。
4. `Industrial` 相关赛道则更强调工位节拍、物料流、装配精度、异常恢复和流程对接。
